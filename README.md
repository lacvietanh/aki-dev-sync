...
| **In-app Terminal** | A `TERMINAL` tab beside the event log running a real interactive shell on this Mac - a genuine PTY, so history recall, `Ctrl+C`, interactive prompts and `vim` all work. One shared session: whatever you type on the Mac or on a paired phone lands in the same shell and both screens show identical output, which is the point - `Terminal.app` renders nowhere but the Mac, so a phone could never drive it. A slim key row (Esc, Tab, sticky Ctrl, arrows, Enter) covers what a phone keyboard can't send, and recent output replays when you reopen the tab or your phone reconnects. The shell is a login shell, so it has the same `PATH` as every other terminal on the machine. Session controls sit in the panel header - **CLEAR** (wipes the Mac's stored scrollback, so it stays clear on the phone too), **RESTART**, **KILL**, and **OPEN** (hands the shell's *current* directory to `Terminal.app`). When the shell exits the terminal says so and turns red instead of going quiet; typing anything starts a new one. Each project's OPEN popup also has **In-App Terminal**, which drops the shared shell into that project. See [In-app Terminal](docs/feat/in-app-terminal.md). |
