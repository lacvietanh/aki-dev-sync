...
| Feature | What it does |
|---|---|
| **Project Tasks** | Per-project task list in a centered modal dialog (TASKS column, right before GIT). Track active, pinned (📌), and wish (🕒) tasks with independent toggles. Pinned tasks sort to the top, wish tasks sink to the bottom of the active list, and completed tasks sink to the absolute bottom with a smooth Vue transition animation. Includes a "Hide Completed" toggle filter. Stored in `projects.json`. |
