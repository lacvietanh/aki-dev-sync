...
- **Hide a project from the list.** Project Settings gained a *Hide from the projects list* checkbox for a project you are done looking at. It is view-only, on purpose (the Logic Pro track-hide model): a hidden project keeps syncing, keeps refreshing its git and remote-diff state and keeps counting its terminals exactly as before, so revealing it shows live state rather than a stale row - and hiding buys no polling or quota savings, which is the trade being made deliberately. While at least one project is hidden, an eye button with a count badge appears in the PROJECTS header to reveal them; no project hidden, no button, so the header gains nothing in the normal case. The *hidden* flag itself is shared with a paired phone (it says something about the project), but the reveal switch is per screen, like terminal text zoom - which is what lets the phone reveal and unhide a project the Mac hid, without going back to the Mac.

- **New app-icon menu item: Antigravity Allowlist.** Merges a checked-in, recommended set of commands (`git status`, `curl`, `jq`, `agy` itself, ...) into the selected host(s)' `~/.gemini/antigravity-cli/settings.json` `permissions.allow`, so a new machine or a new agy account stops getting a permission prompt for every routine dev command. A union merge via `jq`, not an overwrite - every other key in that file (`agentMode`, `model`, `statusLine`, ...) survives untouched, and re-running it is harmless (the list is de-duplicated). Its own menu item, its own modal, and its own backend command (`apply_gemini_allowlist`) - deliberately not folded into the Statusline Customizer, since applying a statusline must never silently widen a host's permissions as a side effect (SRP).
...
- **The Global Note's text box starts at ~2 rows, and its modal follows the app's narrow mode.** It now passes `rows="2"` to the shared notes field and carries no min-height override, so the browser's native textarea sizing sets the floor instead of a fixed pixel value; the modal was also the one modal missed by the narrow-mode padding pass, so it kept full desktop padding on a phone. Dragging it taller still works and the 60vh ceiling is unchanged.
