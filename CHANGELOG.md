...
- **Responsive breakpoint query mismatch in AppHeader and modals.** Replaced container queries `@container main-view (max-width: 700px)` with `@media (max-width: 700px)` in `AppHeader.vue` and `TaskListPanel.vue` since those components reside outside the main-view container.
