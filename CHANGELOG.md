...
- **Agent Usage: tab buttons lost the `src-tab` class and the `.tab:disabled` style** - a disabled tab is no longer dimmed and no longer shows a `not-allowed` cursor.
- **Agent Usage: removed the `Math.max(tierCount, 1)` floor** in `AgentUsageSection.vue` and `AgentUsageSlot.vue` - `tierCount === 0` now renders zero rows instead of one.
- **AppHeader: the Remote HTTPS link no longer depends on `remoteHttpsAvailable`**, only on `remoteHttpsEnabled && remoteHttpsUrl`.
- **`main.css`: removed the entire `.btn-ui-action` rule set** (base, `:hover`, `:active`, `.error-state`, `.error-state:hover`) with no replacement.
- **ProjectTable narrow mode tightened**: TASKS column `2.5rem` -> `2.0rem`, `.last-action` `7px` -> `9px` with `line-height: 1.2`, `.col-terminal` `margin-left: -6px` -> `0`.
- **Repo-wide `[WRAP]` / `[YAP]` formatting sweep and large-scale comment streamline across 131 files (-6,330 lines net: +2,405 insertions, -8,735 deletions)**. Systematically cleaned hard-wrapped prose (`agent.C3`) and removed redundant WHAT/HOW comment narration (`coding.B4`), keeping domain reasons (WHY), doc references (`docs/...`), spec § markers, test vectors, and CLI/API contracts. The sweep was not purely cosmetic: some `style(*)` commits also carried behavior changes, listed separately in this section.
  - **Research & Plan Documentation (15 files, -1,558 lines net)**: `docs/research/test-case-check-flow.md` (unwrapped 631 lines across 98 use cases), `docs/plan/done/wish-terminal-split-simpleview.md` (moved to `done/` with Right-Dock shipped in 1.25.0 and SimpleView rejected/superseded), `docs/research/terminal-input-jul31.md`, `docs/plan/done/terminal-input-jul31.md`, `docs/plan/done/remote-control.md`, `docs/plan/remaining-1.22.md`, `docs/plan/done/remaining-1.25.md`, `docs/plan/done/1.20.0-terminal-and-remote-sync.md`, `docs/index.md`, `docs/plan/done/restore-terminal-mobile-ux.md`, `docs/plan/done/wish-terminal-manual-resize-authority.md`, `docs/plan/done/remaining-1.23.md`, `docs/research/audit-terminal-split-wish.md`, `docs/research/simpleview-mobile-regression.md`.
  - **Rust Backend Layer (15 files, -1,267 lines net)**: `src-tauri/src/web_server.rs`, `system.rs`, `pty.rs`, `statusline.rs`, `sync.rs`, `project_notes.rs`, `projects.rs`, `claude_cleanup.rs`, `git.rs`, `global_note.rs`, `claude_profile.rs`, `logger.rs`, `lib.rs`, `gemini_allowlist.rs`, `ssh.rs`.
  - **Composables Layer (26 files, -1,067 lines net)**: `usePtyTerminal.js`, `usageMonitor.js`, `useTerminalTabs.js`, `useTerminalTextDrain.js`, `useProjectConfig.js`, `useBackgroundRefresh.js`, `useProjectNotes.js`, `useSync.js`, `usageMonitorRegistry.js`, `agUsageCache.js`, `useExternalTerminals.js`, `useRemoteControl.js`, `useCompanionPairing.js`, `useLogs.js`, `useGit.js`, `useProjectTasks.js`, `useTaskCollection.js`, `useTerminalPanel.js`, `useVisualViewportHeight.js`, `usePtyStream.js`, `useTerminalChrome.js`, `useProjectStack.js`, `useSsh.js`, `useSyncStatus.js`, `useTerminalViewType.js`, `useGlobalNote.js`.
  - **Store & Services Layer (22 files, -986 lines net)**: `remoteActions.js`, `terminalTabsStore.js`, `projectStore.js`, `bridge.js`, `ptyBridge.js`, `mirror.js`, `dialogStore.js`, `hostInvoke.js`, `usageMonitorStore.js`, `action.js`, `projectNotesStore.js`, `usageSlotStore.js`, `usageReadingStore.js`, `logStore.js`, `noteStore.js`, `usageTierStore.js`, `syncCheckStore.js`, `intents.js`, `claudeModeStore.js`, `refreshStore.js`, `sshStore.js`, `services/index.js`.
  - **Vue Components, Modals & Global Styles (28 files, -878 lines net)**: `src/App.vue`, `src/assets/main.css`, `src/boot/roleStamp.js`, `ProjectTable.vue`, `TerminalView.vue`, `AppHeader.vue`, `AgentUsage.vue`, `AgentUsageSlot.vue`, `AgentUsageSection.vue`, `ClaudeSettingModal.vue`, `ClaudeCleanupModal.vue`, `ExternalTerminalsModal.vue`, `TerminalTabStrip.vue`, `DialogHost.vue`, `PairingGate.vue`, `SimpleView.vue`, `TaskListPanel.vue`, `TerminalStack.vue`, `UsageCircle.vue`, `TaskCell.vue`, `CountBadgeWrap.vue`, `TerminalScopeButton.vue`, `TerminalChromeMenu.vue`, `TerminalCountBadges.vue`, `ProjectTasksModal.vue`, `SshConfigModal.vue`, `ClaudeProfileModal.vue`, `LogStack.vue`.
...
- **Android/Gboard IME double-insert in the in-app terminal** (`"ăn gì"` → `"aăn giì"`, `"báo"` → `"baoáo"`). Gboard corrects an already-committed syllable via a plain `deleteContentBackward` against a textarea `useTerminalTextDrain.js`'s drain had already emptied, so the delete silently vanished and the correction's `insertText` just appended onto text the PTY already had. The drain now translates a `deleteContentBackward` landing on an empty textarea into a real backspace byte instead of dropping it. Hardware-confirmed on Android/Gboard: the correction is not composition-wrapped. See `docs/research/terminal-gboard-double-insert.md`.
...
- **External-terminal Tauri command surface replaced.** Removed `count_external_terminals`, `count_external_terminals_global` and `list_external_terminals`; added `list_terminal_sessions` and `describe_terminal_sessions`, backed by the new `TerminalOwnership` state.
- **Tab strip pin/close buttons are easier to hit and keyboard-reachable**: larger click targets, `aria-label`s, Enter activation; the pin tooltip now reads "Unpin - keep in this group only", and the close icon turns red on hover.
- **Agent Usage and ProjectTable switched from `@media` to `@container main-view` queries** (`AgentUsage.vue`, `AgentUsageSection.vue`, `AgentUsageSlot.vue`, `UsageCircle.vue`, `ProjectTable.vue`) - the opposite direction to the AppHeader/TaskListPanel fix below, because these components live *inside* the `main-view` container while those live outside it.
- **The OPEN popup's hover area extends 12px further down** (`.open-popup::before`), so the pointer no longer loses the popup in the gap.
- **The dock splitter highlights from its own `:active`** instead of an ancestor `.is-dragging` class.
- **IntroModal copy:** "Force Delete" -> "Mirror / Delete", "Logo menu" -> "App-icon menu".
- **New maintenance script, `scripts/fix-ssh-agent-leak.sh`, for the `~/.zshrc` ssh-agent leak this app's login shells amplify** (see `docs/ref/ssh-agent-leak.md`); not run by the app itself, an owner runs it manually against their own `~/.zshrc`; defaults to a dry run and only writes with an explicit `--apply`, after a timestamped backup.
...
- **The event log's Copy button no longer drops its "copied" state early** when pressed repeatedly (`useLogs.js` now clears the previous timer before scheduling a new one).
- **State mirroring over WebSocket.** Filtered out readonly computed refs from mirror registration and added ephemeral SSH modal state (`sshStore.showSshModal`, `sshConfigText`, undo/redo) to `PER_SCREEN_KEYS` to avoid cross-device modal state bleed.
...
- **The in-app terminal panel now collapses itself once every tab, in every group, is closed** (a tab's own ✕, ⌘W on the last one, or any other close path), instead of sitting open over an empty mount area. Collapsing and expanding the panel (this one and the event log panel, which shares the same CSS rule) now eases smoothly instead of snapping instantly.
- **The statusline's rate-limit cache (`rate-limits-cache.json`) now respects a custom `CLAUDE_CONFIG_DIR`.** It was hardcoded to `$HOME/.claude`, unlike the `.claude.json` read next to it which already followed `CLAUDE_CONFIG_DIR` — a machine with that variable set to something else read the account's identity from one config dir and its cached rate limits from another.
- **"Open SSH in-app" now always opens a new terminal tab.** It used to find an existing tab for that project with `runKind: 'ssh'` and focus it instead, so a second SSH tab for the same project was impossible. The dedup is removed — DEV/BUILD still dedup by their own `runKind` unchanged. The per-scope (5) and global (16) tab caps still apply and still show the existing toast when hit.
- **Pasting text into the in-app terminal and then pressing SPACE no longer duplicates the pasted text.** xterm.js clears its hidden input textarea before the browser inserts the pasted text rather than after, so the app's input drain later re-sent the leftover text along with the next keystroke. The drain in `useTerminalTextDrain.js` now clears that textarea itself as soon as a paste is detected, before any later keystroke can drag the residue along.
...
- **Antigravity usage probe ported from JavaScript to POSIX shell (`scripts/get-antigravity-usage.sh`) and Rust backend decomposed into domain modules**:
  - `scripts/get-antigravity-usage.js` ported to POSIX `sh` (`get-antigravity-usage.sh`), removing the `Interpreter::Node` interpreter variant, `NODE_BIN_RESOLVER_PREAMBLE`, `run_remote_node_timeout`, and dead `"antigravity-usage"` dependency from `package.json`.
  - Script output protocol converted to delimited JSON frames (`|||AGPROC|||...`), with JSON parsing, model quota extraction, and multi-account snapshot assembly moved to pure Rust (`antigravity_payload.rs`).
  - Monolithic `agent_usage.rs` (892 lines) decomposed into `src-tauri/src/remote_shell.rs` (transport funnel & SSH lock) and `src-tauri/src/agent_usage/` domain submodules (`mod`, `probe_result`, `probe_log`, `claudecode`, `antigravity`, `antigravity_payload`, `antigravity_logout`).
  - Dedicated exit code `3` introduced for missing required remote tool (`curl`), producing a visible one-time `ag_tool_missing_once` log entry. IPC command signatures and payload shapes remain byte-identical.
...
- **The three files the Antigravity-usage-probe refactor (`610fd93`) had already deleted in git but never deleted from the working tree are now actually gone**: `src-tauri/src/agent_usage.rs`, `scripts/get-antigravity-usage.js`, `docs/plan/usage-probe-oop.md`. Their presence on disk made `src-tauri/src/lib.rs`'s `mod agent_usage;` ambiguous between that file and `agent_usage/mod.rs` (rustc E0761) on any checkout carrying them, which is a build-breaking state, not a cosmetic one. Full account, including how it fooled an eight-agent audit before being caught: `docs/research/ghost-files-and-audit-method-jul30.md`. `scripts/test/monitor-ag-proxy.js`'s dev-only command now points at the live `.sh` probe instead of the deleted `.js` one (its JSON-parsing still needs updating for the probe's new framed-output protocol, noted inline - this script is not on any shipped path).
- **DEV and BUILD now launch into the in-app terminal instead of an external `Terminal.app` window**, which is what makes them visible from a paired phone for the first time (`docs/plan/done/dev-build-in-app-launch.md`). Each press opens or focuses a tab tagged `dev`/`build` in that project's scope: a live tab is only focused, never re-sent the command (a finished build's output stays on screen, a running dev server is never interrupted); an exited tab is respawned and the command re-sent automatically; otherwise a fresh tab opens, never the scope's own last-active shell, so build/dev output can never interleave with whatever you were mid-typing there. Every freshly-spawned terminal tab's shell now also gets common macOS install directories (`~/.local/bin`, `/opt/homebrew/bin`, `/usr/local/bin`, `~/.claude/local`) prepended to `PATH` before its own login-shell rc-sourcing runs, closing (for those locations) the same PATH cold-start race this project has hit before — an `nvm`-managed Node install is a known, stated exception this does not cover. `run_project_command`/`run_project_dev`/`run_in_project_terminal` (`src-tauri/src/system.rs`), left over from the old external-Terminal path, are removed along with their `hostInvoke.js` allowlist entries and `lib.rs` registrations - nothing called them anymore. Verified on macOS by the owner, 2026-07-30: DEV/BUILD open/focus an in-app tab, a live tab is never re-sent the command, an exited tab respawns and re-sends it, and the popup renders correctly.
...
- **DEV and BUILD are no longer missing from the OPEN popup for projects the stack detector does not recognise.** Both buttons were removed from the menu entirely whenever no command resolved - which is every project that is not Tauri, Nuxt or plain Node (Python, Go, a static site, anything without a `package.json`). Nothing on screen then hinted that the feature existed or that Project Settings has a field to fill in, so the only way back was already knowing. They now always render, dimmed, with a tooltip naming the problem and the next step; a per-project override still wins over the detected default, and a blank override still falls back to it.
- **The project name in the "type the project name to confirm" delete dialog can now be selected and copied.** The app disables text selection everywhere by default and opts specific things back in; this phrase - the one string the user is being asked to reproduce exactly - had been missed, so a name with dashes or mixed case had to be retyped from sight. The gate itself is unchanged: the typed value is still verified on the Mac.
- **On a phone, opening the on-screen keyboard no longer covers the terminal's compose input.** The whole app shell was sized in the CSS `vh` unit, which iOS Safari does not shrink when the keyboard opens (only the visual viewport does) - so a fixed-height layout never adjusted and the keyboard simply covered whatever sat at its bottom edge, which was always the compose input the user had just tapped. A `visualViewport`-driven CSS variable now feeds every `vh`-based height (`main.css`, the dock's height/maximize math) and tracks the keyboard on iOS Safari, iOS "Add to Home Screen", and Android Chrome alike; the Mac host window is unaffected since it has no on-screen keyboard to react to. Real-device confirmation across all three phone surfaces is still outstanding (`docs/plan/done/terminal-mobile-keyboard-viewport.md`).
...
- **DEV and BUILD no longer echo their command twice on a freshly opened tab.** The pending command could land in the tab's initial output snapshot and then be appended a second time once live output arrived, so the terminal showed the same `npm run …` line twice for a first spawn. It is now sent once, after the tab has finished hydrating; a respawn of an already-dead tab is unaffected.
- **Antigravity usage now finds a CLI (`agy`) session listening on a fallback port.** The CLI probe still tried HTTPS then HTTP on one port before giving up, unlike the IDE/Desktop probe already fixed to try HTTPS across every candidate port first; a CLI session on the next port over returned no data until this was brought in line.
...
- **TERMINAL column** on the project table replaces LAST ACTION: one button opening/reactivating that project's terminal group (works from a phone), with two overlay badges - top (cyan, red if a shell has exited) = tab count in that group, bottom (slate) = external `Terminal.app` windows standing in that project's directory right now.
- **LAST ACTION** moved under the sync buttons as two small muted lines (action + time, host on its own line) instead of its own column - absent entirely for a project that has never synced.
- **Global Note gained a full task list**, sharing the same task engine, list UI and notes field as per-project tasks. Still stored as `{content, tasks}` in `globalnote.json`.
...
- **A remote path that cannot be resolved over SSH now reports the error** instead of launching VSCode at a `vscode://…/~/project` URI built from the unresolved path.
- **`open` failures are visible**: `macos_open` waits for the process and returns its error, so a missing app or an unhandled URI raises a Toast instead of looking like success.
- **DEV/BUILD no longer claim "Command started in Terminal!" for a folder that is not there** - the host refuses before opening a window when the project's local directory does not exist (same guard on the popup's Terminal item), and a second click on DEV/BUILD is ignored while the first is still in flight.
- **Every LOCAL popup item is dimmed when the project's folder is missing** (unmounted volume), with an explanatory tooltip. COPY stays enabled.
- **IDE availability is re-checked each time the popup opens** rather than once per reload, and a not-yet-loaded result now reads as *unavailable* instead of enabling every IDE entry.
- **COPY works on the phone**: `navigator.clipboard` does not exist over plain http, so there is now a textarea fallback, and a total failure says so instead of failing silently.
- **REPORT from a phone now confirms itself** ("Report opened on the Mac") - the browser window opens on the Mac, which the phone cannot see.
- **The terminal-tab limit is enforced where the user is**: the cap is checked before the tab action is sent, so a phone gets the "limit reached" Toast instead of a tap that does nothing.
- The dock's **"open in Terminal.app"** button now pokes the external-Terminal scan like the popup's Terminal item, so the badge updates at once rather than on the next 5s tick.
- The false **"TERMINAL - EXITED"** red header on open: liveness is now three-state (`unknown`/alive/exited), so a terminal never flashes red before its first real status arrives.
- The on-screen key row (Esc/Tab/Ctrl/arrows/Enter) is now companion-only - it no longer appears on the Mac's own screen, which has a real keyboard.
- **The usage cards now show real numbers on the phone.** The readings lived in a place the host never mirrored, so a companion had no data - and its own attempts to fetch produced SSH probes on the Mac every time the phone regained focus. Fetching is host-only now, and the readings are shared state like everything else.
- **Usage monitors stop when nothing is watching them.** Pointing a slot at ten hosts in one session left twenty SSH poll loops running for the rest of the session, on cards nobody could see and whose stop control was therefore unreachable. Monitors are now held by reference count; the last release stops the timer and keeps the last reading, so switching back repaints instantly.
- **A stale usage number no longer reads as current.** Past a 5-hour boundary with no new activity, the card kept showing the pre-reset percentage with no age and no stale badge, indefinitely. Staleness is derived from the reading's timestamp now, so it cannot be forgotten on any path.
- **Unreadable probe output no longer replaces a good reading with an empty card.** Malformed cache JSON was being reported as a successful, empty result; it is now a miss, so the last good numbers stay on screen.
- **A Claude Code host that cannot be reached keeps its cached card** instead of replacing it with an error banner on every poll, matching how the Antigravity card already behaved.
- **A host without `python3` is no longer silently unmonitorable**: three unguarded command substitutions in the usage probe aborted the whole script under `set -e`, with the reason recorded only in a debug log that a shipped build never enables.
- **The exited-shell badge on a project row never appeared.** It read a reactive value the wrong way in script code (correct in templates, which is why the tab strip's copy worked), so the condition was always false.
- **COPY LOGS and task copy were dead on the phone** for the same reason COPY was - no clipboard API over plain http. All four copy paths now share one implementation with a working fallback and a visible failure.
- **Tapping COPY on the phone no longer flashes the Mac's COPY button**, and the log console's element reference is no longer broadcast as shared state (it could never be, and failed on every session).
- **A refused terminal tab can no longer steal the phone's focus later.** When the host refused a tab at the cap, the phone's pending claim stayed set indefinitely and grabbed the next tab opened in that group, possibly hours later. The claim now expires.
- **The log panel no longer closes under you** after a sync: the 1.5s auto-close checked which project's log was open before starting its timer, not when firing, so opening another project's log inside that window closed it.
- **Hovering the OPEN column no longer fires an IPC call and a broadcast per row.** IDE availability is host-machine state that changes when an app is installed, not on mouse-over; it is cached for 60s and only written when the answer actually changed.
- **The app no longer freezes on an unmounted network volume.** Directory checks for sync, terminal-open, stack detection and the project load at startup ran on the thread that dispatches UI calls, where a stale mount blocks for the mount's timeout. They now run off that thread.
- **The companion stays connected after a slow call.** Every remote call was cut off at 20s, including SSH-backed ones with a 30s budget, so the phone reported the host as unresponsive while the host was finishing normally. Budgets are per command now.
- **Pairing survives an internal error.** Two locks in the companion server were taken in a way that made a single panic permanently break pairing and new connections, contradicting the rule the module's own comment states.
...
- Moving popup position into the component touches only that one field: every other `projectRuntime` key (sync state, git counts, epochs) is untouched, and removing one project cannot disturb another's popup position.
- The tab-cap check kept in the store as well as the caller, so a direct host-side call is still refused.
- The live external-Terminal count is a snapshot rebuilt wholesale from ONE scan per tick, so no project's badge can be cleared on another project's behalf; a failed scan leaves the previous counts standing rather than zeroing every badge.
- Closing one terminal tab leaves every other tab's shell and scrollback intact.
- Closing a project's last terminal tab drops you back to the global group and leaves every other project's tabs untouched (and the global group itself, whose own floor of ≥1 tab is unaffected).
- A notes-only Global Note save leaves the global task list untouched (and a task edit leaves the note text untouched).
- Editing global tasks cannot touch any project's tasks.
- Old `globalnote.json` files (content only) still load - the task list simply starts empty.
- Two Global Note edits arriving at once (Mac and phone) can no longer lose one of them: the read-modify-write is serialised, and it still patches only the fields present, so a content save leaves the task list intact and a task edit leaves the note text intact.
- Restricting what a phone may call did not remove any working companion feature: PUSH/PULL, terminal, project commands, SSH config editing, usage and the global note all still work from the phone, because each already routed through a host-side action or a read-only command.
...
- **Recent output replays** on reopen/reconnect after a screen lock; output is coalesced (~20ms/16KB) so a chatty build doesn't flood the connection.
- **Console panel header now has LOG/TERMINAL tabs**; behavior is unchanged if you never open the terminal.
- **Terminal session controls** in the panel header: CLEAR (wipes stored scrollback, not just the visible screen), RESTART, KILL, OPEN (hands the shell's current directory to `Terminal.app`).
- **"In-App Terminal" added to every project's OPEN popup**, above `Terminal.app`, jumping straight into that project's directory (path is shell-escaped).
- **Shell starts as a login shell** (`-l`, `TERM=xterm-256color`) so nvm/rbenv/`path_helper` behave the same as any other terminal.
- **Usage monitors are now per-slot, not app-wide**: each slot's REMOTE host picker is independent, so you can watch several machines/accounts at once (Claude Code and/or Antigravity).
- **Existing monitor and slot settings carry over on upgrade** (on/off switches, LOCAL/REMOTE tab, AG/CC choice); old settings are read, never deleted.
...
- Deleting a Claude Code account and creating a new one under the same email no longer shows the old account's quota bar/plan badge; the cache now keys on the account's stable id, not email alone.
- Terminal no longer renders as a tiny box in an empty panel (mount/fit-on-font-load issue fixed).
- Exiting the shell (or a crash) now shows `[process exited]`, turns the tab red, and RESTART works, instead of leaving a dead, silent session.
- Quitting the app now tears down the in-app terminal's shell and its whole process group instead of leaving it running.
- Task notes/checklist edits made from a phone no longer revert after a reconnect; edits now apply to the Mac's live state, not only to disk.
- Confirmation dialogs (typed delete, preview-failed, remove project, missing SSH host, upload overwrite, Antigravity log out) are now shared state: visible and answerable from either screen.
- Antigravity monitoring of a remote host now works when Node is installed via nvm (binary is located directly instead of relying on a non-login SSH shell's PATH).
- Remote Antigravity card no longer shows this Mac's own numbers; a card only displays a reading provably its own.
- Claude Code `default_claude_ai` tier no longer renders as "Ai"; falls back to the subscription type.
- Antigravity account menu no longer loses hover moving from the trigger to the menu.
- A REMOTE card with no host picked no longer shows local numbers, and its power button no longer toggles the local monitor. Every other monitor's on/off state and reading is preserved.
- A remote monitor you'd switched off no longer silently re-enables itself after upgrading.
- Overwrite confirmation for "Upload (select files)" now appears on both screens, not just the Mac.
- Starting a sync while the TERMINAL tab is open now switches to the LOG tab instead of hiding the log.
- Opening REPORT.html no longer freezes the window while pulling a newer remote copy.
- A project with an empty local/remote path is now refused at save and at sync time instead of resolving to `/`.
- Folder names containing a quote or `$(...)` no longer execute as shell commands (Terminal, SSH terminal, remote path resolution, DEV/BUILD).
- A failed remote check (unreachable host, non-GNU `stat`) is no longer treated as "nothing there"; it now fails the call instead of enabling an unsafe overwrite or delete.
- File lists in the delete confirmation dialog render as text, not HTML.
- A failed delete preview now requires typing the project name to confirm, instead of a plain "continue anyway".
- "Upload (select files)" no longer silently no-ops while DRY RUN is on.
- Remote Antigravity card no longer mixes up IDE vs CLI sessions signed in under the same email; readings are keyed by machine, account and session type. No existing cached reading is lost.
- Restarting the app no longer forces every paired phone to re-pair; revocation and "temporarily offline" are now distinct close codes, and the On/Off switch is remembered across restarts.
- Closed a privilege-escalation gap over Tailscale HTTPS where another machine on your tailnet could take over the Mac's host role via the shared `127.0.0.1` proxy address without a pairing token.
- Two confirmations queued back to back no longer leave the app stuck "syncing forever"; confirmations now queue and are answered in order.
- Restarting a terminal from one screen while the other screen still thought it was dead can no longer kill the fresh session.
- A syncing project's PUSH/PULL button is now a STOP button that kills the transfer and its ssh; quitting the app kills any transfer still running.
- SSH connections now time out after 10s and rsync after 120s of no data, instead of hanging indefinitely.
- A remote directory name containing a space is no longer created as two directories.
- An unmounted project path (shown as "No Git") no longer allows a sync into an effectively empty folder; it gets its own amber badge and blocks sync until it's back.
- The usage-monitor circuit breaker now correctly detects a refused connection, and a tripped breaker actually stays stopped instead of being retried by the wake-up watchdog.
- Visiting another remote host no longer clears the account pinned to a usage slot.
- Turning on Claude Code proxy mode no longer permanently disables local monitoring after you leave proxy mode.
- Per-screen UI state (open log panel, per-slot host) is no longer broadcast to other screens; project data, logs, dialogs and the terminal stay shared.
- Failed remote-control commands now surface a toast instead of failing silently.
- Sync logs are capped at 2,000 lines per project, and only new lines are sent to connected phones, instead of resending the whole log on every line.
- The terminal on a phone now scales to fit instead of re-wrapping the Mac's output to the phone's width.
- A restart no longer strands the previous shell's last output above a cleared scrollback.
- A project added/edited mid-session now gets its icon immediately, and editing its folder updates the icon too.
- A project deleted on one screen can no longer be resurrected by saving from a stale config window still open on another screen.
- Renaming a host in your SSH config no longer silently repoints projects onto a different server; the app now asks and lists the affected projects.
...
- A remote host name beginning with `-` is now rejected instead of being passed to `ssh`/`rsync` as an option.
- Files this app writes (project list, global note, Claude Code `settings.json`, `.zshrc`, statusline scripts) are now written atomically (temp file + rename) so a crash can't truncate them; `settings.json`/`.zshrc` keep a timestamped backup on every write, not just the first.
...
- **Per-slot account selection persists** across reloads.
- **Per-field truncate widths, zebra background and print order are generated from the UI**, including the "separate" padding toggle.
- **Host rows show a small `CC`/`AG` tag per CLI actually set up on that host** — filled when installed and wired up, hollow amber when installed but not wired.
- **Window presets (width + placement) can be remembered across launches** via a "remember" checkbox in the ☰ menu; off by default.
- **`⌘1` / `⌘2` apply a whole window layout in one keystroke** (Narrow+Stick-Top-Left / Wide+Center-Primary), shown as badges in the menu.
...
- Statusline Customizer layout: CLI target checkboxes moved into the modal title, added Check all/Uncheck all, host section renamed "Target Hosts" to avoid clashing with the "Apply to" name.
- Statusline is now one shared script for both CLIs (it detects which CLI is running it from its own install path), so quota pools and Claude Code's reset countdown that the old per-target generator dropped now render for everyone.
- The statusline script (`src-tauri/src/statusline-unified.sh`) is a real file compiled into the app; Apply only rewrites its generated config block, so the shipped and read versions can no longer drift.
...
- Auto-install now repairs the CLI it actually probed for (was probing Claude Code but repairing AGY).
- Account field now populates for Claude Code (falls back to `~/.claude.json`) and for AGY (fixed to read `.active` instead of `.[0].email`).
- AGY quota no longer shows `-2400%` (percent vs. fraction readings are both handled now, clamped 0-100).
- AGY reset ETA no longer always shows `Xh0m` (minutes are now passed to the format string).
- AGY no longer invents a model name when the payload has none.
- Statusline field delimiter changed to Unit Separator, so a value containing spaces no longer shifts later fields.
- Removed dead `rate_reset_*` variables that were leaking into the output pipeline.
- `get-claudecode-usage.sh` now applies the same v4 expiry/account-scope checks as the write side, so an older host without the new hook can't feed the app a phantom quota.
- Deduplicated Antigravity accounts in the cached account list (one record per email now, newest/most-complete wins); every other account is untouched.
- Account lookup no longer matches by prefix (`an@x.com` no longer resolves to `another@x.com`).
- A usage slot pointing at a since-logged-out account now falls back to the live source instead of showing stale data.
- Applying the statusline to AGY CLI now actually registers it in AGY's `settings.json` (previously wrote the file but never pointed AGY at it).
- Fixed the statusline rendering completely blank when the CLI reported its model as a plain string instead of an object.
- Model names no longer start with a stray dash when the CLI reports a raw id.
- Usage-row 1/2-row buttons now visibly show which is selected.
- CWD color picker now actually changes the CWD color.
- Switching off a parent toggle (Cache, 5h/7d limit) no longer erases the child settings underneath it.
