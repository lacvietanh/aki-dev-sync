...
#### Fixed
- **Orphaned `claude` sessions no longer accumulate on the remote host** (`force-sync-claudecode.sh`, `get-claudecode-usage.sh`, `provision-claudecode.sh`, `agent_usage.rs`, `useAgentUsage.js`): a remote host was found holding **19 `claude` sessions several days old, 6GB RAM + 4GB swap**. The initial investigation (`docs/research/ssh-process-leak-remote-ram-overflow.md` §1–10) blamed the 30s usage poll loop; re-checking against the logs disproved that and the doc now carries a correction (§11). Force-sync was the real source, via three compounding defects. **(1)** Force-sync runs three sequential `claude -p` API round-trips (usage → probe → usage) but went through the same `run_remote_script` budget as a single poll — 30s. It was structurally impossible to finish, so it was killed mid-flight on nearly every run; it now has its own 180s budget. **(2)** No `claude` call was time-bounded *on the remote*, and killing the local SSH does not reliably kill a grandchild through a login shell — a `claude` stalled on an API call simply ran forever. Every `claude` invocation is now wrapped in a 45s bound injected via the shared script preamble, so it ends itself and leaves nothing to clean up. Since stock macOS ships neither `timeout` nor `gtimeout` (verified), there is a `perl -e 'alarm shift; exec @ARGV'` fallback — `exec` replaces perl with `claude` itself so the signal lands on the real process, not a wrapper; end-to-end tested against a hung command inside `zsh -lc`, leaving zero stray processes. **(3)** The cleanup sweep used `pkill -f "claude -p"`, which **never matched anything** — the real command line is `claude --model haiku -p /usage`, in which the substring `claude -p` does not occur. Cleanup had been silently doing nothing since it was written, while still opening an extra SSH connection each time. Patterns are corrected and now matched against real command lines. Because a process that ends itself needs no cleanup, `pkill` is demoted to a safety net for hosts with neither `timeout` nor `perl`.
- **Usage polling stops instead of probing a dead host forever** (`useAgentUsage.js`, `agent_usage.rs`): the same incident log shows 12 consecutive failures spaced at exactly 30.0s — the poll kept going for 24 minutes after the host had stopped accepting TCP. After 5 consecutive failures polling now halts and says so, resuming only on an explicit user action (Reload or a host change) — deliberately not on the `visibilitychange`/`focus` wake listeners, which fire constantly and would rebuild the same loop through the back door. A hard stop rather than exponential backoff: the log's even spacing proves probes were already serialized by the existing `isChecking` guard and never piled up, so there was nothing for a graduated delay to relieve. Separately, the post-timeout cleanup SSH — previously fire-and-forget with *no timeout at all*, added to a host by definition already struggling — is now bounded at 8s and does all its patterns in one round-trip, and every polling SSH carries `ConnectTimeout=10`, `ServerAliveInterval=5`, `ServerAliveCountMax=3` and `BatchMode=yes` (without `ConnectTimeout` a saturated host could burn the whole script budget on the TCP handshake alone, timing out before anything ran remotely).
