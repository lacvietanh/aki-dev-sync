...
#### Fixed
- **Usage polling no longer hammers an unreachable remote into RAM exhaustion** (`useAgentUsage.js`, `agent_usage.rs`): a remote host was found saturated with orphaned SSH/`claude` processes, traced to a positive feedback loop in the usage poller (full analysis: `docs/research/ssh-process-leak-remote-ram-overflow.md`). The poll interval and the remote-script timeout were both 30s, so each `setInterval` tick fired a new SSH at the exact moment the previous one was killed — the host never got an idle moment, and every killed probe could strand 2–8 child processes remotely (SIGHUP does not reliably reach a `python3`/`claude` grandchild). Over 24 minutes that is ~50–100 connections and hundreds of orphans, and nothing ever stopped: the loop kept probing at full rate long after the host had stopped accepting TCP at all. Four changes, each closing one leg of the loop: **(1)** polling is now a chained `setTimeout` rather than a fixed `setInterval`, so the next probe is scheduled only once the previous one has finished and probes can never overlap however slow the host is; **(2)** after 3 consecutive failures the interval backs off exponentially up to a 10-minute ceiling, reset the instant the host answers or the user hits Reload — the watchdog's suspend-detection threshold now derives from that same backed-off delay, so it can't re-fire probes every 7s and defeat it; **(3)** the post-timeout orphan cleanup, previously a fire-and-forget `ssh … pkill` with *no timeout at all* (an unbounded blocking call added to a host that is by definition already struggling), now has an 8s bound and does all its patterns in one round-trip instead of one connection per pattern; **(4)** every polling SSH now carries `ConnectTimeout=10`, `ServerAliveInterval=5`, `ServerAliveCountMax=3` and `BatchMode=yes` — without `ConnectTimeout` a saturated host could burn the whole 30s script budget on the TCP handshake alone, timing out before anything ran remotely. Cleanup deliberately still targets only this app's own command patterns (`claude -p`, `claude auth status`, `claude --model haiku`) and never a blanket `pkill python3`/`node`, which would kill unrelated work on a shared dev box.
