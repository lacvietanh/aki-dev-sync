...
- **Terminal keyboard input now goes through an app-owned textarea overlay instead of xterm.js's internal event pipeline.** The old IME guard (`useWkImeGuard.js`, 1.22.0) intercepted xterm's internal handlers — inherently fragile across xterm versions. The new approach sets `disableStdin: true` on xterm (turning it into a pure renderer), hides its textarea, and overlays a transparent app-owned `<textarea>` that captures all keystrokes through the browser's own (correct) event pipeline. This solves both OpenKey-style key-injection engines (whose synthetic keystrokes WKWebView tags `keyCode 229`, which xterm 5.5.0 IME-fallback paths drop/collapse — upstream #5887/#5894, unfixed) and true composing IMEs whose preedit the browser now draws natively on the overlay. Diagnostics (`window.__akiTermInput`, replacing `__akiIme`) and an escape hatch (`localStorage['aki-input-mode']='legacy'` to revert to the old guard) are both kept. **Preserved**: the compose row for true IMEs, the old guard file for the legacy path, and the research docs as the root-cause record.
