...
- **One shared session, not one per device.** Whatever you type on the Mac and whatever you type on the phone land in the same shell, and both screens show byte-identical output. That is deliberate: the goal was to reach *the terminal you are already using*, not to open a second one with a different working directory and history. The PTY is the single source of truth for what is displayed, so neither screen echoes a keystroke locally before the shell has echoed it, and the Mac is the sole resize authority so a phone-sized viewport can never reshape the Mac's output mid-build.
- **A compact key row for what a phone keyboard cannot send** - Esc, Tab, a sticky Ctrl (tap Ctrl, then tap C), the four arrows and Enter, in one slim row of icon buttons. Ordinary typing needs nothing special: the on-screen keyboard already produces real per-character input.
- **Recent output replays automatically** when you reopen the terminal or your phone reconnects after locking, so a screen lock does not cost you the context of a running build. Output is coalesced before it goes on the wire (roughly every 20ms or 16KB) so a chatty build does not turn into one network message per read.
- **The console panel's header is now two tabs.** That header used to be titled by whatever it was showing; a small log icon and a terminal icon now sit at its far left and switch between the two, with the title text following behind them - so "GLOBAL EVENT LOG" has moved one step to the right rather than gone anywhere. Nothing was added to the screen for this: the tabs took the space the title gave up, and the buttons on the right still change with whatever is showing. If you never open the terminal, the log behaves exactly as it always did.
- **Terminal session controls, in the panel header that was already there** - CLEAR, RESTART, KILL and OPEN. CLEAR wipes the Mac's stored scrollback rather than just the screen in front of you, so it stays cleared on the phone too and does not come back on the next reconnect. RESTART kills the shell and starts a fresh one. OPEN hands the shell's *current* directory - wherever you have `cd`'d to, not where it started - to `Terminal.app`.
- **"In-App Terminal" on every project's OPEN popup**, above the existing `Terminal.app` item, jumping the shared shell straight into that project's directory. This is the one of the two that works from a phone, which is the whole reason the in-app terminal exists. The path is single-quote escaped, so a directory containing spaces or quotes cannot turn into a command.
- **The shell starts as a login shell** (`-l`, with `TERM=xterm-256color`), so it reads the same startup files every `Terminal.app` window does. Without that, nvm/rbenv/`path_helper` never run and the in-app terminal quietly has a different `PATH` from every other terminal on the machine - the same class of surprise as the Antigravity-over-SSH bug fixed below.

- **Every usage monitor is now its own thing, so you can watch several machines at once.** Point one slot at Claude Code on one server and another slot at Claude Code on a second server, each signed into a different Claude account, and both show their own numbers side by side. The same goes for Antigravity, and for mixing the two. Until now the app could hold exactly one remote machine: the REMOTE tab in every slot read a single app-wide host picker, so choosing a host moved every remote card at once — and each one threw away its reading on the way. The host dropdown that was already in each slot's header now belongs to *that* slot; nothing new was added to the screen. See [the refactor plan](docs/plan/usage-monitor-entity-refactor.md).
- **Your existing monitor and slot settings are carried over, not reset.** The four on/off switches the old app-wide picker owned are read once and re-expressed as one switch per monitor, so a source you had turned off stays off after the upgrade - the two remote ones are matched to whichever host was selected at that moment, the only host they could ever have described. Each slot's remembered LOCAL/REMOTE tab and its AG/CC choice are lifted the same way. The only thing that cannot be carried over is which machine a slot watches, because a slot could not hold one before; every slot therefore starts out following the app-wide host picker, which is exactly what all of them did until now. The old settings are read, never deleted, so going back to 1.19.0 finds them intact.
- **Switching a monitor off no longer loses where it got to.** It keeps its last reading on screen, marked as cached, and picks up again when switched back on. Turning one monitor off has no effect on any other, on any machine.
- **A Claude Code card that has gone quiet now tells you how long for, instead of just saying "Stale".** The word on its own gave you nothing to act on — a reading two minutes past the freshness line and one from yesterday morning looked identical. It now reads `12m ago`, the same way the Antigravity card has always shown the age of a cached reading, and hovering gives the absolute time. The number is not an estimate: it is the very timestamp the card was already using to decide it had gone stale, so what you see and what the app believes cannot disagree. Where the machine cannot report that timestamp at all, the old badge still appears rather than a guessed age. Nothing was added to the screen — it replaces the badge in place, and the plain text is narrower than the badge's own padding and background were.

#### Fixed
- **A monitor forgot everything whenever you changed which machine it watched.** Changing the remote host discarded the previous host's reading outright, so even *taking turns* between two servers meant starting from nothing each time. A monitor can no longer be re-pointed at all — the machine is part of what a monitor *is* — so there is nothing left to discard. The specific guards that existed only to cope with a monitor whose target could move (`realHostChange`, `lastNonNullHost`, and the several "is a machine even selected" checks scattered through the poll loop) are deleted rather than disabled.
- **Two machines signed into the same Antigravity account overwrote each other's cached usage.** The cache filed a reading under its account address and noted the machine only as a side remark, so both machines wrote the same entry: whichever polled last won, and the other card — correctly refusing to show a number that wasn't provably its own — went empty. The machine is now part of how a reading is filed. The account dropdown had the sharper version of the same bug: its de-duplication pass kept the newest record per account **across every machine** and deleted the rest, so a fresh poll on one server destroyed the other server's history for that account. It now only ever compares records from the machine it is listing. Existing cached readings are re-filed under the machine that took them, and no reading is dropped in the process. Two things still write to this cache outside a poll - the ten-day expiry sweep, and that same de-duplication pass, which runs whenever the dropdown is opened rather than only when a reading arrives; both are described under *Upgrade note* below so the scoping claim above is not read as absolute.
- **The remote card could show another machine's numbers even after 1.20.0's earlier fix.** That fix scoped the *lookup*; this one scopes the *storage*, which is where two records sharing one key could still collide. A slot also no longer parses the cache itself — it asks the one module that owns it, which is what makes the scoping enforceable rather than repeated in two places.
- **Deleting a Claude Code account and creating a new one under the same email kept showing the old account's weekly quota bar and plan badge.** Both the cache that survives between CLI turns and the gate meant to keep one account's numbers off another account's card identified an account by email alone — and a freshly created account can share an old one's email while being a different account entirely (new organization membership, new plan, no weekly limit at all). The email still matched, so nothing ever told the cache the account had changed: a phantom 7-day bar sat frozen at whatever it read on the last turn of the old account, and the plan badge kept reading the old plan long after the new one was active. The cache now keys on the account's own stable id instead, falling back to email only for a cache written before this account id existed. A field that stops being sent at all — like a weekly quota that a new plan simply doesn't have — now ages out after a few hours of silence rather than surviving indefinitely just because its reset time hadn't arrived yet. The plan badge also stops trusting the account's on-disk credentials file first; that file is only ever refreshed at login and had been the one place still holding the previous account's plan hours after the switch, so the two live sources (the account's own config, then its cached auth status) are checked first and the credentials file only as a last resort.
- **The terminal rendered as a small box in the corner of a large empty panel.** Its container sat in a row-direction flex parent without a grow factor, so it sized itself to its content - an empty div at mount time - and the fit measured a near-zero width, locking the terminal at its minimum size. The mount now fills the panel, and fitting is deferred by a frame and re-run once web fonts finish loading, since a fit against an unresolved layout produces exactly the same wrong answer. Fits that measure an absurd size (a collapsed panel, a tab mid-transition) are now discarded rather than pushed to the real shell, where they would permanently re-wrap a running build's output.
- **Exiting the shell froze the terminal with no explanation and no way back.** Typing `exit` - or `exit` twice out of an `ssh` session - ended the shell, but the app kept the dead session in place: keystrokes went into a closed pipe, nothing appeared, and there was no visible sign anything had happened or any control to recover. The shell ending is now a real state: the terminal prints `[process exited]`, the tab header turns red, RESTART lights up, and typing anything starts a fresh shell. This is also what happens if the shell crashes. A restart racing a slow exit cannot kill the new session - each shell carries a generation and only retires the slot if it still holds its own.

- **Quitting the app left the in-app terminal's shell running.** Closing the app ran no cleanup on the terminal at all — `KILL` and `RESTART` were the only two things that ever ended a shell, and both are buttons you have to press. Quit with the terminal open and the shell, plus anything still running in it, was simply handed over to the system. The app now tears the terminal down on quit, and the teardown signals the shell's whole process group rather than only the shell, so a command you left running goes with it instead of being left to fend for itself. Note this is about the in-app terminal specifically; the usage monitors' own SSH connections were never in this path and are unchanged.
- **Task notes and checklist edits made from a phone reverted a while later.** The edit reached the Mac's disk but never its live in-memory state, so the next time the phone reconnected - a screen lock, a backgrounded tab, a brief network drop - the Mac replayed its own stale copy straight back over the edit. Every task, note and drag-reorder change now applies to the Mac's real state first, addressed by project id, so it survives the next reconnect. Only the one project being edited is touched; the others are not rewritten.
- **A confirmation dialog triggered from the phone appeared only on the Mac.** Starting a PUSH or PULL with `--delete` from a phone left it watching a spinner that never finished, while the Mac quietly held a modal waiting for someone to walk over and answer it. Confirmations are now shared state: the same dialog appears on both screens, either one can answer it, and whichever answers first wins. This covers the typed delete confirmation, the "preview failed, continue anyway" prompt, Remove Project, and the missing-SSH-host replacement picker. The typed project name is still checked on the Mac, so answering from the phone cannot skip the safety check.
- **Antigravity monitoring of a remote host never updated.** The probe ran `node` over SSH through a shell that does not read your shell startup files, so a Node installed by nvm was simply not found, and the failure looked identical to the ordinary "nothing running right now" case - which is why it stayed invisible for a whole release. The remote Node binary is now located by checking well-known install paths directly before falling back to the PATH, and a genuine "not found" is written to the log once per host instead of blending into the noise.
- **The remote Antigravity card showed this Mac's own numbers.** Cached readings were filed by account email alone, and the local and remote monitors share one cache, so the remote card kept rendering whatever the local machine had last measured. Each cached reading now records which machine it came from, and a card only displays a reading that is provably its own; a remote host with nothing measured yet shows an empty state instead of a number from somewhere else. Cached data for other accounts is left exactly as it was.
- **Claude Code `default_claude_ai` rate limit tier rendered as "Ai" badge** (`AgentUsage.vue`): Claude Code credentials updated by Anthropic set `rateLimitTier: "default_claude_ai"`, causing `claudeTierDisplay` to trim `default_claude_` to `"ai"` and capitalize it to `"Ai"` instead of falling back to `subscriptionType` (`"Pro"`). Added a check so an `"ai"` tier string falls back to `subscriptionType` when available.
- **Antigravity Account Menu lost hover when cursor moved from email trigger to menu box** (`AgentUsage.vue`): The 4px positioning gap (`top: calc(100% + 4px)`) lacked an invisible bridging pseudo-element, causing the menu to vanish (`visibility: hidden`) instantly when moving the mouse across the gap. Reduced gap to `2px` and added an 8px transparent `::after` bridge over the space, matching `.icon-dropdown` and `.open-popup` hover pattern.
- **A REMOTE card with no machine picked yet showed this Mac's numbers, and its power button switched the local monitor off.** A slot whose REMOTE tab had no host selected resolved to an empty host, and an empty host was being read as "this Mac" — so that slot and the local one were literally the same monitor: one instance, one reading, one saved on/off switch. Slot D starts on REMOTE, so a fresh install with no SSH host configured hit this immediately: the card claimed to be remote while showing local numbers, and pressing its power button turned off local monitoring instead. Depending on which card the app built first, the local monitor could also come up permanently switched off with no way to switch it back on. "No machine picked yet" is now its own identity, and its power button does nothing rather than saving a switch nobody reads. **What is preserved:** every real monitor keeps its own saved on/off state and its own reading, on this Mac and on every host — the fix adds one new identity and changes no existing one, so nothing stored under `claudecode@local`, `antigravity@local` or any `…@<host>` is read, rewritten or cleared by it. Switching one monitor still has no effect on any other.
- **A remote monitor you had switched off could come back on by itself after upgrading.** The one-time carry-over of the old on/off switches runs at startup and needs to know which host the two remote switches described — but the SSH host list is read from disk a moment later, so on a machine that had never pinned a host, both remote switches were simply unreadable at that instant and were dropped. A monitor with nothing saved defaults to on, so a remote monitor deliberately turned off came back on and started polling that host over SSH again. The carry-over now waits for the host to resolve and then reads exactly those two switches, and only fills in a switch that has no saved value — no other monitor's setting is read or written.
- **The overwrite confirmation for "Upload (select files)" appeared only on the Mac.** This was the same defect as the `--delete` confirmation fixed above and had been missed: pick files to push from a phone, and if any of them already exist on the remote, the phone sat on a spinner while the Mac silently held a modal asking whether to overwrite them. It is now shared state like every other confirmation — visible and answerable from either screen, first answer wins — and the whole SELECT flow (native file picker, conflict check, push) now runs on the Mac when triggered from a phone, rather than being attempted on the phone. The Antigravity "log out" confirmation had the same problem and is fixed the same way.
- **Starting a sync while the TERMINAL tab was open sent the log into a hidden panel.** The console panel opened but kept showing the terminal, so a running sync looked like a frozen app. Starting a sync now selects the LOG tab, and cancelling at the confirmation puts the tab back where it was.
- **Opening a project's REPORT.html could freeze the window.** When the remote's copy was newer, the app pulled it with `rsync` on the same thread that handles UI calls, so a slow or unreachable host froze the window for as long as `rsync` took. The pull now runs off that thread, like every other network call in the app. This one predates 1.20.0.

<!-- The block below came out of a pre-release audit: 98 things a user actually does, each traced
     back through the code rather than clicked. See docs/research/test-case-check-flow.md. -->
- **A project with an empty local or remote path would have mirrored your entire filesystem.** Clearing either path field and saving left a project that rsync resolved to `/` — and since the deletion side is on by default for PULL, the first PULL would have mirrored the remote directory over the root of the Mac. Nothing stopped it at any layer: not the config modal, not the save, not the sync command. Save is now refused with the offending field named, and both sync directions refuse independently, so a `projects.json` edited by hand cannot get through either. A project whose drive simply is not mounted is still perfectly valid — that is a disk to plug in, not a broken project, and turning it into a validation error would only hide the real reason.
- **A folder name containing a quote or `$(...)` ran as a command.** Opening Terminal, opening an SSH terminal, resolving a remote path or running DEV/BUILD pasted the path into a shell line wrapped in double quotes, which do not stop substitution — so a directory called `it's $(whoami)` executed. Every one of those call sites now goes through a single quoting function, and remote paths through a variant that keeps `~`/`$HOME` expandable on the far side while quoting everything else. Naming a folder with an apostrophe or a space is normal; making it work is the program's job, not the user's.
- **A failed remote check was indistinguishable from "the remote has nothing there".** If the SSH probe died — host unreachable, or simply a non-GNU `stat` on a BSD/macOS remote, which fails while ssh still exits 0 — every file came back marked "does not exist, timestamp 0". Both readers of that answer treat it as *safe to overwrite* and *safe to auto-approve a delete*, so "we could not find out" was being filed under the most destructive possible interpretation. The probe now works on GNU and BSD remotes, and any file it cannot answer for fails the whole call instead of guessing. The overwrite prompt for "Upload (select files)" therefore stops being skipped when the host is down.
- **The list of files about to be deleted was rendered as HTML.** A filename containing markup was interpreted rather than shown — in the one dialog whose entire purpose is to display exactly what is about to be destroyed.
- **When the preview failed, the confirmation got weaker instead of stronger.** A dry run that could not enumerate the deletions offered a plain "continue anyway", skipping the type-the-project-name step that the same operation requires when the preview *succeeds*. Not knowing what will be deleted is the moment to be most careful, so that case now falls into the typed confirmation, titled to say the file list is unknown. It is one gate, not two: a second prompt stacked on the first gets read as paperwork and clicked through.
- **"Upload (select files)" silently did nothing while DRY RUN was on**, despite the tooltip promising it was unaffected. You picked files, confirmed the overwrite, and read "Sync complete" — with nothing sent.
- **The remote Antigravity card could show another session's numbers.** A machine can be signed in through the IDE and through the desktop/CLI at the same time under one email — two sessions, two separate quotas. The cache filed both under the email alone, so saving one deleted the other, and a card pinned to the IDE session was served the CLI session's figures. A cached reading is now identified by machine, account **and** session type, and the account list went back to being a read that changes nothing. Where the app cannot prove a reading belongs to the session on screen, the card stays empty rather than showing a number: you glance at a quota to decide whether to keep going, and a plausible number from the wrong session sends you into a limit mid-task, while an empty card just makes you press Reload. **What is preserved:** no cached reading is deleted or rewritten by this change and the storage version is unchanged, so upgrading loses nothing; only the "last used account" pointer is migrated, and where it is ambiguous it is dropped rather than guessed — costing at most one poll.
- **Restarting the app made every paired phone pair again.** Remote Control always started switched off, and one close code meant both "your token is rejected" and "remote control is off" — so the phone read an ordinary restart as being revoked and threw its token away. The two cases are now separate codes: a phone that is merely waiting for the Mac keeps its token and reconnects by itself, and only a genuine revocation clears it. The On/Off switch is remembered across restarts, which does not weaken "off means off" — what is restored is your own last decision, never a default the app chose, and a fresh install still starts off. The pairing code is deliberately *not* remembered, so a code read off the Mac's screen last week is already dead, and the ten-wrong-codes lockout is remembered so it cannot be cleared by waiting for a restart.
- **With HTTPS over Tailscale on, another machine on your tailnet could take over the Mac's own role.** The host role was allowed to anyone connecting from `127.0.0.1`, but `tailscale serve` proxies every tailnet connection through exactly that address — so a peer could claim it, cut the real Mac out of its own session and feed connected phones whatever state it liked, without a pairing token. The host role now also requires a secret minted fresh in each app run and given only to the Mac's own window: a proxy can forge an address, not a value it was never handed. This is outside the posture declared for 1.19.0, so it is a fix, not a trade-off.
- **Two confirmations at once left the app stuck syncing forever.** The second dialog replaced the first, and the first — a promise nobody would ever settle — left that project marked as syncing until the app was restarted. Confirmations now queue and are answered in order. They wait silently: someone reading a list of files about to be deleted is holding the one decision that cannot be undone, and telling them another question is stacked up behind it splits exactly the attention the dialog exists to hold.
- **Typing into a terminal whose shell had exited could kill the shell that had just replaced it.** Restarting from one screen never told the other screen the shell was alive again, so a tap on the phone was still read as "the shell is dead, restart it" — killing the Mac's fresh session and wiping the scrollback with it. The live/dead state is now announced to both screens whenever it changes. Beyond that, a keystroke on a dead terminal now *starts* a shell instead of restarting one: if the flag is ever stale again the worst case is nothing happening, rather than destroying a running session. Whoever is holding the phone in another room did not ask to destroy anything.

#### Upgrade note
- **Upgrading rewrites the stored Antigravity usage history, and going back to 1.19.0 afterwards empties it.** Every cached Antigravity reading is re-filed under the machine that took it, which is what makes two machines on one account stop overwriting each other. Nothing is lost in that move - the account, the numbers and the time each reading was taken all survive - but it is a move, not a copy: the old file is removed once the new one is written, and 1.19.0 can only read the old one. Run 1.20.0 once and then roll back, and every Antigravity card starts empty until it has polled again; the readings are gone, not hidden. This is deliberate, because leaving the old copy behind would let a rollback-and-return quietly resurrect readings filed under the wrong machine - the exact bug this release closes. Everything else you have stored is untouched by the rewrite: Claude Code readings, your account and slot selections, SSH hosts, projects, tasks and notes live elsewhere and are neither read nor written by it, and a rollback finds all of them as they were.
- **Two things in the Antigravity cache still act across machines, on purpose.** A reading is discarded once it is ten days old no matter which machine took it - an expired reading is expired for everyone, and it was already too stale to be shown. And opening the account dropdown is not a pure read: while listing accounts it also collapses duplicate rows for the same account, which means it deletes records. Both are kept as narrow as they can be - the expiry only ever removes readings past their age limit, and the dropdown only ever compares and collapses records belonging to the machine it is listing. Every other machine's records, and every account other than the duplicated one, come through both untouched.

#### Security note
- **Opening the in-app terminal from a paired device adds no new confirmation step, and that is a decision, not an oversight.** A paired device could already invoke any command on the host, including the one that runs an arbitrary shell command for DEV and BUILD - so gating a PTY alone while that stays open would be theatre, and it would defeat the entire purpose, since the phone is used precisely when nobody is at the Mac. The pairing token remains the single gate; **Off** still cuts every live device instantly. This extends the same posture already declared in 1.19.0 rather than quietly widening it.

#### Known limitation
- **A phone's Project Config modal does not close itself after a remove triggered from that phone.** The removal itself does happen on the Mac and mirrors everywhere; only the modal's own open/closed state is still per-screen, because it has not been moved into shared state yet. Nothing is lost and nothing is left half-removed.

#### Not yet verified on a Mac
- Most of this batch was written on a Linux box that cannot compile the app; it has since been compiled and unit-tested on a Mac. `cargo check` is clean (0 warnings), `cargo test --lib` passes 106/106, and the frontend build passes — which settles the Rust terminal backend and every `portable-pty` API shape it was written against. The audit fixes above added 29 of those tests, including a shell-quoting suite run against a real shell rather than string comparison, and a multi-account cache suite that was first shown to *fail* on the pre-fix code so it proves something. What is still open is the packaged build and live behaviour: `npm run build:rmud` has not been run, so `portable-pty` has never been linked against a universal build, and no live two-screen test has been done. The specific checks to run are listed in [the 1.20.0 plan](docs/plan/1.20.0-terminal-and-remote-sync.md) §6 and, for the multi-machine monitors, [the entity refactor plan](docs/plan/usage-monitor-entity-refactor.md) §6 — which include watching two hosts at once under two accounts, and confirming that retargeting one slot leaves every other slot's reading intact.
...
- **Antigravity monitoring for a remote host.** The usage panel's REMOTE tab now carries the same `AG | CC` pair as LOCAL, each tab with its own power icon doubling as that source's on/off state; the SSH host picker shrank to make room and is disabled only when both remote sources are off. One host choice, two independent monitors of it. Like the other three sources it is on by default, so an upgrade with a host already selected starts polling it - the same per-interval SSH cost the Claude Code remote monitor has always had; turn it off from its own power icon if you don't want it. Log Out is withheld while a card shows a remote host: those commands act on this Mac and would sign out the wrong machine.

#### Changed
- **A pool whose 7-day quota is full now dims its own 5-hour reading.** Once the weekly quota reads 100% the pool is exhausted whatever the 5h figure says, so that figure is dropped from the colour ladder and dimmed instead of competing for attention; the tooltip says why. Scoped strictly per pool - Antigravity's Gemini and Claude/GPT pools and Claude Code's single pool each dim only their own 5h, and a missing or N/A weekly reading dims nothing.
- **The statusline's `<20%` tier is readable again, and the app's swatches now show the terminal's real colour.** The bottom tier moved from bold blue (`01;34`, close to unreadable on a dark background) to aquamarine (`01;38;5;86`) - brighter, still visibly colder than the green tier above it. The underlying cause of the mismatch is fixed too: the modal used to draw the ladder with Tailwind hexes while the script printed ANSI codes, so app and terminal disagreed at *every* tier. Both sides now derive from one `{ key, ansi, hex }` table where the hex is that ANSI code's real xterm rendering, and every swatch in the dynamic-colour area carries its code in the tooltip (`aqua - ANSI 01;38;5;86 (#5FFFD7)`) so the two can be compared instead of trusted.
- **The two "Apply to" checkboxes look like checkboxes.** They were filled violet while every other checkbox in the modal is orange-brown, which read as a colour swatch - a value being picked - rather than a toggle.
- **Text selection is off across the app chrome and on where it matters.** Labels, tab captions, table headers and badges no longer highlight on an accidental drag; console output, git status and diff, changelog and release notes, project paths, account emails, the pair code and the statusline preview all stay selectable. Three global rules replace a default plus seven per-component declarations.

#### Fixed
- **The 700px narrow-mode block had been dead code since 1.14.0.** It sat above the base rules it was meant to override, at equal specificity, so the cascade discarded all of it - only `.u-narrow-hide` (which carries `!important`) ever took effect, and the modal chrome trim shipped in 1.14.0 never once applied. Moved to the end of the stylesheet, with a comment stating it has to stay there.
- **Modals were never audited at narrow widths.** At ~420px: GitModal's five footer buttons overflowed the row, ProjectTasksModal forced a horizontal scrollbar at *every* width (a flex item wrapping an `<input>` had no `min-width: 0`), IntroModal's two-column feature grid shredded prose into two-word lines, ProjectConfigModal's paired path fields left ~180px each, and a long project name in any modal title pushed the close button off screen. Fixed per modal, with anything common to three or more lifted into one global rule.
- **A second narrow breakpoint had drifted in.** ProjectConfigModal carried its own `560px` block; 700px is the single app-wide value and now genuinely is.
...
#### Known limitation - investigated, not an app bug
- **DEV / BUILD / REPORT still switch macOS Spaces** when the browser or Terminal already has a window on another desktop. The app cannot control this: per Apple's Mission Control documentation a *new* window also opens on the Space where that app already has windows, and there is no public API to place a window on a chosen Space. The fix is on the machine - `System Settings → Desktop & Dock → Mission Control →` turn off *"When switching to an application, switch to a Space with open windows for the application"*, and check the app's Dock icon → Options → Assign To is **None**. Every approach tried (`open -n`, `open -g`, per-app `open -na`, AppleScript window-then-activate) and why each fails is recorded in [the 1.19.0 plan](docs/plan/done/1.19.0-final.md).
...
#### Added
- **Dual statusline deployment adapter**: the Statusline Customizer now generates and deploys from one config to two targets behind a `StatuslineTarget` trait - **Claude Code** (`~/.claude/statusline-command.sh` + the `statusLine` key in `settings.json`) and **AGY CLI** (`~/.gemini/antigravity-cli/statusline.sh`).
- **CLI identity tag**: every statusline can now lead with a colored `CC` or `AG` tag (orange-brown bg for Claude Code, dark navy bg for AGY, white text) so a glance tells you which CLI produced the line when both write to the same terminal. On by default, toggle-able like any field, but pinned first with no drag handle - it always stays at the very start of the line.
- **Account field in the statusline catalog**: `account` (active session email) is available in both the Rust generator and the Vue customizer, with a user-editable color.
- **Persistent slot account selection**: the account picked per usage slot (`slotViewingEmail`) survives a reload via `localStorage` (`aki-usage-slot-${slotId}-viewing-account`).
- **Per-field truncate widths, zebra block background and print order are generated from the UI**, including the "separate" padding toggle.
- **Each host now shows which CLI it is actually set up for.** A host row carries a small `CC` / `AG` tag for every CLI found on it - filled when that CLI's statusline is installed *and* pointed at, hollow amber when the CLI is there but nothing is wired up yet. Previously one warning triangle spoke for Claude Code alone, so a host set up for AGY looked identical to one set up for nothing.
- **Window presets can be remembered across launches.** The four window buttons in the ☰ menu now sit under an `AppWindow:` heading with a **remember** checkbox on its right. With it on, the width preset (Narrow/Wide) and the placement preset (Stick Top-Left/Center Primary) are stored separately and both re-applied on next launch - width first, since Stick Top-Left fits its height to the current width. The remembered preset on each axis is highlighted in the menu. Off by default, and switching it off clears what was stored.
- **`⌘1` / `⌘2` apply a whole window layout in one keystroke** - `⌘1` = Narrow + Stick Top-Left, `⌘2` = Wide + Center Primary. They work without opening the menu, and each key is printed as a badge on the seam between its column's two buttons, so the pairing is visible rather than documented.
- **`npm run test:statusline`** - an automated stand-in for opening the customizer, ticking one option, pressing Apply and reading the written file by eye. It feeds the customizer's own settings through the generator, runs the resulting script against fixture payloads and checks the rendered line, covering all 18 switches plus the widths, colors, shades, thresholds and drag order. An option that stops reaching the file now fails here rather than on a user's machine - the same check that would have caught the dead CWD color picker. It found the blank-statusline bug below on its first run.
- **Every Terminal window this app opens auto-snaps to the top-right corner.** Local terminal, SSH terminal, DEV/BUILD, and the AkiClaudeDoc installer all route through one launcher, which now resizes the new window to 124 columns and positions it flush against the top-right of the main display, height filling the space between menu bar and Dock. The move/resize goes through System Events' Accessibility API (`AXPosition`/`AXSize`), not Terminal's own scriptable `bounds` property - `bounds` silently refuses to move a window's Y coordinate onto a different display than the one it opened on (X moves fine, Y and height stay near their original values), which `AXPosition`/`AXSize` does not.

#### Changed
- **Statusline probe/apply is faster and safer for the hosts it touches**: the per-host SSH timeout for the Customizer's probe and Apply dropped from 30s to 5s (this work is a few local file writes, not an API call), the remote shell itself now self-terminates via `timeout`/`gtimeout`/a `perl` fallback even if the local side gives up first (previously a killed local SSH could leave an orphaned remote process running unbounded), every host now runs in its own thread instead of one after another (Apply to 3 hosts no longer takes 3x as long), and every host is now locked against this app's *other* features touching it at the same time (the auto-install probe and a manual Apply could previously fire overlapping SSH connections at the same host).
- **Antigravity surface labels shortened**: the three header titles locked in 1.17.0 are now `AG` (Desktop App), `IDE` (VS Code extension) and `CLI` (terminal) - previously `AG`, `AG IDE` and `AGY`, with a fourth `AG / AGY` label for a merged desktop+CLI session that now simply reads `AG`. Log Out button text and the account-dropdown tooltips follow the same three names.
- **Header dropdown reorganised**: Changelog moved up to the version row (it is a link, not an action), Check Update took its place in the button row and now disables itself while a check is in flight, a separator after the Usage Row controls segregates usage-row options from window-resize actions, and the Statusline Customizer row carries a Claude Code + Antigravity icon pair on the right so the two targets it writes are visible before opening it.
- **Statusline Customizer layout**: the "Apply to" CLI target checkboxes moved up into the modal title (they govern the whole modal, not the host list), the field list gained a **Check all / Uncheck all** toggle, and the host section - previously also called "Apply to" - is now "Target Hosts", so the two different "apply" choices are no longer named the same thing.
- **Statusline: one script, not two generated dialects.** Claude Code and AGY CLI now receive the byte-identical script; it works out which CLI is running it from its own install path instead of being generated per target. The `3p-*` quota pool (Claude/GPT models running inside AGY) and Claude Code's reset countdown, both of which the old per-target generator dropped, now render for everyone.
- **The statusline script is no longer assembled from strings in Rust.** `src-tauri/src/statusline-unified.sh` is a real, runnable file compiled into the app, and Apply rewrites only its `AKI-GENERATED-CONFIG` block from the customizer's settings. A test regenerates that file from the UI defaults and compares it byte-for-byte, so the version shipped and the version read can no longer drift - the failure mode that produced three divergent scripts before 1.18.0.
- **Customizer defaults have one owner.** The Vue component is the source of truth; `default_config()` and the `get_default_statusline_config` command are gone, along with the call that overwrote the UI's defaults with them - the reason the customizer could show colors the running statusline never used.
- **Host rows are tighter and checkboxes no longer read as color swatches.** The rows lost the padding a pill shape forced on them, and every checkbox in the modal is now a hollow box with a drawn check instead of a filled light square sitting next to the color swatches.

#### Removed
- **Removed the standalone `share/aki-statusLine/` copy of the statusline script** (and its README section). It was a hand-maintained duplicate of what the generator already produces, loaded by nothing at runtime - an earlier round of fixes landed there instead of in the generator and silently never reached the app. The Statusline Customizer is now the only way this script is produced.

#### Fixed
- **Phantom quota in the statusline (rlcache v4) - a bug that shipped in every release from 1.10.0 through 1.17.0, not a new regression**: the rate_limits cache introduced in 1.10.0 had no expiry check and was shared by every account that ever ran on the host, and because the merge only ever adds or overwrites keys present in the live payload (never dropping absent ones), any field that landed in the cache once lived there forever. Field evidence: a host displayed `7d 45%` for an account that has no weekly limit at all - leftover data from a *different* account's session. The cache is now bound to the account that wrote it (`~/.claude.json` -> `.oauthAccount.emailAddress`; a foreign or unlabelled cache is discarded instead of displayed) and any entry whose `resets_at` has already passed is dropped rather than shown. `resets_at: 0` still means "unknown", not "expired", so a live quota with no reset timestamp is preserved. A `DESIGN LOCK` comment now sits at the cache code in both the generator and the reference script, cross-referenced with `docs/plan/done/1.18.0-statusline-apply-correctness.md` P0-5.
- **rate_limits cache lost data three other ways** (same rewrite): a corrupt cache file used to blank the entire statusline, a payload carrying only `five_hour` overwrote the stored `seven_day`, and the write was not atomic. The cache is now validated before use, **merged** rather than replaced - the 7d window survives a 5h-only payload and vice versa - and written via tmp+rename. Field order, colors and thresholds in a saved config are untouched by this change.
- **Apply now writes the targets you ticked**: "Apply to" defaulted to `agy` only, so a plain Apply never wrote `~/.claude/statusline-command.sh` while the button's tooltip still promised it. Both targets are now ticked by default, the choice persists across sessions (`aki-statusline-targets`), the tooltip names the exact files that will be written, and Apply is disabled with none ticked. The backend now rejects an empty target list instead of silently falling back to AGY.
- **Auto-install repaired the wrong CLI**: the host probe looks for the *Claude Code* statusline but auto-install wrote the AGY file and then marked the host configured - the probe failed again on every reopen. Auto-install now applies the `cc` target it actually probed for.
- **Account field on Claude Code was a dead toggle**: Claude Code's statusline payload carries no email, so the field was always empty. It now falls back to `~/.claude.json` -> `.oauthAccount.emailAddress`.
- **Account field on AGY read the wrong shape**: the disk fallback ran `.[0].email` against `~/.gemini/google_accounts.json`, which is an object (`{"active": ..., "old": [...]}`), not an array - the jq error was swallowed and the email never appeared. Now reads `.active`.
- **AGY quota showed `-2400%`**: a numeric quota reading (percent remaining) was run through the fraction formula. Fraction and percent readings are now both handled, and the result is clamped to 0-100.
- **AGY reset ETA always showed `XhOm`**: `printf '%dh%dm'` was passed only the hours. Minutes are now passed too.
- **AGY no longer invents a model name**: a payload without a model rendered a fabricated `gemini 3.6 flash`; the model block now simply disappears, like every other empty field.
- **Statusline field delimiter desync**: the jq field separator is now the Unit Separator (`\x1f`), so a model display name or path containing spaces no longer shifts every field after it.
- **Dead `rate_reset_*` variables in the Claude Code join loop**: the reset toggles are modifiers of the 5h/7d blocks, not fields of their own, and no longer leak an undefined `$g_rate_reset_*` into the output pipeline.
- **Read-side v4 gates for `rate-limits-cache.json`**: the v4 expiry and account-scope gates only protected hosts that had received the new `statusLine` hook - a host still running an older script could feed the app the same phantom-quota shape the write-side fix closed. `scripts/get-claudecode-usage.sh` now applies the same two checks before emitting cache JSON: an entry whose `resets_at` has passed, or is `0`/missing (unverifiable), is dropped; a cache written by a different account is discarded entirely (an unlabelled pre-v4 cache is kept, not dropped, so hosts pending re-apply don't go blank). The reader is strictly read-only - it never writes or deletes the cache file, that stays the statusLine hook's job.
- **Duplicate and mismatched Antigravity accounts in the cached account list**: the same email could occupy several entries in the `localStorage` account store at once (one per `sourceType` it had ever been seen under, plus a legacy key with no suffix), so the dropdown listed one account two or three times and a slot could restore the stale copy. The store now keeps exactly one record per email - the newest `fetchedAt` wins, and a record carrying a live `quotaSummary` wins a tie - with the older keys for *that same email* removed as they are superseded. **Every other account in the store is untouched**: the pruning is keyed on the email being written, never a wipe of the map, so a machine with several accounts keeps all of them (see the multi-entity regression guard in `CLAUDE.md`).
- **Account lookup matched the wrong account by prefix**: `loadAgAccount()` resolved a key with `k.startsWith(emailPart)`, so looking up `an@x.com` could return the record of `another@x.com`. It now matches the key exactly or on the `email:sourceType` boundary.
- **A usage slot could restore an account that no longer exists**: with the selection now persisted across reloads, a slot pointing at an account that has since been logged out and evicted from the cache silently fell back to showing another account's numbers under the old selection. The slot now detects that its selection resolved to nothing, clears it, and returns to the live source.
- **Applying the statusline to AGY CLI now actually shows one.** The script was written to `~/.gemini/antigravity-cli/statusline.sh` but nothing ever pointed AGY at it - its `statusLine.command` stayed empty, so the file sat there unused while Apply reported success. Apply now registers the script in AGY's own `settings.json`, the same way it already did for Claude Code. Hosts applied to before this fix need one more Apply.
- **Statusline could render completely blank against some payloads.** A single unguarded field lookup aborted the whole extraction step when the CLI reported its model as a plain name rather than a structured object, so every value on the line came back empty at once. The lookup is now resolved once, defensively, for all four quota readings.
- **Model names no longer start with a stray dash.** When a CLI reports its model as a raw id rather than a display name (`gemini-2.5-flash`), removing the vendor word left the separator behind and the line read `-2.5-flash`. Leftover punctuation at either end is now trimmed and doubled separators collapsed, so the same id reads `2.5-flash`.
- **The Usage row 1/2-row buttons show which one is selected.** They carried an `is-active` class that no style ever matched, so the active preset looked identical to the inactive one.
- **The CWD color picker changes the CWD color.** It was wired to a variable the script never read, so picking a color did nothing at all.
- **A parent toggle no longer erases its children.** Switching off Cache, or a 5h/7d limit, only hides the group; the individual choices inside come back exactly as they were when it is switched on again.
...
- **Busy state lives on the checks, not on their callers** (`beginRefresh`/`endRefresh`/`isRefreshing`/`anyRefreshing` in `projectStore.js`). A per-project counter (not a boolean - several checks can be in flight for one project) is incremented by each check itself, so a project's Refresh icon spins for work no human triggered, including every background ring cycle. The header's own spinner is derived from the same counters via `anyRefreshing`, so the global and per-project indicators can no longer disagree. An earlier attempt put a `refreshing` flag on a `refreshProject()` wrapper instead; the background timers didn't call that wrapper, which is exactly why the per-project icons stayed inert on their own refresh cycles.
- **In-flight checks are now genuinely cancelled, not just ignored on arrival.** `invoke()` has no abort handle, so cancellation uses a per-project generation token (`epoch`, with `bumpEpoch()`/`currentEpoch()`): each check captures the epoch before awaiting and discards its result if it no longer matches. `bumpEpoch()` also force-resets that project's refresh counter to 0, so the spinner stops the instant the cause fires rather than whenever the superseded call eventually resolves. Bumped when a project's `remote_host`/`local_path` changes (`saveConfig`, which also blanks the now-meaningless `hasPendingPush`/`hasPendingPull` and immediately re-checks against the new host), when sync check is switched off (`toggleSyncCheck`, all projects), and when the project list is re-read from disk (`loadData`). Removing a project cancels its checks by the same mechanism: epochs are guaranteed `>= 1` for a live project, so the `0` reported for a deleted one can never collide with a captured value. **This only ever cancels read-only status checks - an rsync push/pull in progress is never touched.**
- **Turning sync check off/on now tears down and rebuilds the background diff cycle, not just makes it a no-op.** `restartDiffTimer()` (`useBackgroundRefresh.js`) no longer creates its `setInterval` at all while `syncCheckEnabled` is false (previously the timer kept firing regardless and relied on `checkProjectSyncStatus`'s internal early-return to silently do nothing every tick), and a new watcher tears down/rebuilds the timer on every toggle, running one check immediately on re-enable instead of waiting out the rest of the interval.
- **The per-project Refresh button disables (and shows "Sync check is off")** when `syncCheckEnabled` is off, matching the PUSH/PULL fieldset's existing pattern instead of staying clickable while half of what it does silently no-ops.
- **Statusline Customizer threshold labels are icon+tooltip only** ("Yellow ≥/Orange ≥/Red ≥" text removed, colored dot + `title` attr instead) and `loadCfg()` now merges newly-added fields into an already-saved local config instead of the saved config silently shadowing them forever.
- **Narrow mode (≤700px): modal body/footer padding trimmed** in `ClaudeSettingModal.vue` and `ClaudeProfileModal.vue` - scoped-style specificity was winning over the global narrow-mode rule in `main.css`.
...
- **Collapsed Global Event Log is much shorter** (`main.css`, 120px → 92px → 55px after a second round of feedback - the 92px "~2 lines" estimate still read as too tall once visually tested). The expanded height (40vh) is unchanged.
...
- **Regression (introduced 1.9.3): Antigravity account dropdown lost its whole multi-account history on every logout, down to just 1 entry**: 1.9.3 (`a26b8f5`/`b082d0d`) treated "header keeps showing the just-logged-out account until a new one goes live" as a bug and added `resetAccount()` → `clearAgStore()`, which deleted the entire `aki-antigravity-usage-cache-v2` localStorage key on every logout - not just the just-logged-out account, every previously-cached account. Root-caused against the actual product goal (PO decision, `docs/arch/usage-antigravity.md` § Log Out behavior & cache retention): the cache exists specifically so each account's *last-known state* stays visible, since Antigravity can only show one account live at a time - showing the old account's data post-logout is correct, not a bug, and retention is meant to be unbounded. `resetAccount()` now only triggers an immediate recheck; it clears no state at all. `clearAgStore()`/`clearLastActiveAg()` and the forced blank (`data`/`activeEmail`/`viewingEmail` reset) are both removed.
...
- **`viewingEmail` pin trapped UI on wrong account when AG switched accounts**: if the user had pinned a previous account (X) in the dropdown while the live Antigravity account was Z, and then AG switched to a completely new account (Y), every subsequent live fetch of Y was blocked by the `viewingEmail === activeEmail` gate - `data.value` never updated and the header showed X's stale cache forever with no escape short of manually clicking Y in the dropdown. Fix: when a live fetch returns an `activeEmail` different from the previous poll's `activeEmail` (i.e., the underlying account changed), `viewingEmail` is auto-reset to `null` so the UI follows the new live account immediately.
- **Remote actions disabled state incomplete / patchy**: project remote sync controls (`SELECT`, `.git`, `PUSH`, `DRY`, `PULL`) previously relied on individual `:disabled` bindings that omitted the `.git` and `DRY` toggles, leaving them clickable when Remote Mode was turned off. Wrapped the entire remote action group in a native `<fieldset disabled>` (`ProjectTable.vue`) to cleanly lock all controls inside without boilerplate repetition.
- **RefreshRing countdown continued animating when remote checks or quota monitoring were disabled**: the SVG countdown ring in the project ACTIONS column header and quota reload widgets kept spinning its interval timer even when Remote Mode (`!remoteModeEnabled`) or a monitoring source (`sourceOff`) was turned off, giving a false visual signal of background network polling. Updated `:interval-s` bindings (`ProjectTable.vue`, `AgentUsage.vue`) to pass `0` when disabled so the ring cleanly pauses/hides.
...
- **De-duplicated remote-path resolution in `ProjectTable.vue`**: `openIdeRemote`'s inline `~`/`$HOME` resolution is now `resolveRemoteFullPath(host, path)`, shared with the new remote COPY button instead of being reimplemented.
