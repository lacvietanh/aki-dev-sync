...
#### Added
- **Window-view shortcuts `F1` / `F2` / `F3`, plus a third width preset at 1400px (`Ultra`).** `F1` applies Narrow + Stick Top-Left, `F2` sets the width to 1400px, `F3` centers on the primary monitor; the same three actions are on chips in the App-icon menu's `AppWindow:` grid, and `Ultra` joins `Narrow` / `Wide` as a clickable width preset with its own active state. Every width preset is now capped at the current monitor's work-area width before resizing, so 1400px on a smaller display fills the screen instead of hanging off the right edge. Replaces the previous `⌘1` / `⌘2` combos.
