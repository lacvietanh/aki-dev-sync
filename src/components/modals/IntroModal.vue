    ...
    <div class="modal-body scrollable intro-content u-select-text">
      ...
      <div class="features-grid mb-3">
        <div class="feature-card">
          ...
          <div class="feature-text">
            <strong>PUSH</strong>
            <span>Push Local → Remote with everything not listed in <code>push_excludes</code>, plus a per-project "Mirror / Delete" setting. A folder listed only in <code>pull_excludes</code> is <strong>push-only</strong> - sent up, never pulled back, never counted as a change (<code>.git/</code> by default: the AI gets full history, and the PUSH badge stays dark through git's own housekeeping). While a sync is running that project's button becomes <strong>STOP</strong> - it kills rsync and its ssh, and quitting the app kills them too.</span>
          </div>
        </div>
      ...
      <div class="features-grid">
        ...
        <div class="feature-card">
          ...
          <div class="feature-text">
            ...
            <span>The power icon in the SYNC column toggles PUSH/PULL/SELECT, the Open popup's <strong>Upload (select files)</strong> item, and sync diff checks (background + manual) - the popup's REMOTE column itself stays available, since SSH Terminal and the remote IDE entries only reach the server, they do not sync files. In Agent Usage's REMOTE tab, the <code>AG</code> and <code>CC</code> tabs each have their own power icon, toggling the Antigravity and Claude Code monitors independently. Switching one off never switches off another. Every monitor is on by default; each active remote monitor costs one SSH round-trip per refresh cycle, and its power icon stops it completely. If a host stops answering, that monitor gives up rather than probing a dead machine forever: its power icon turns <strong>amber</strong>, the tooltip names the host, and clicking it retries. A project disabled in Project Config skips its own background sync/git polling; its manual Refresh, PUSH/PULL and git actions keep working.</span>
          </div>
        </div>
        ...
        <div class="feature-card">
          ...
          <div class="feature-text">
            ...
            <span>Checks for updates silently on launch, or manually from the App-icon menu, showing a version badge and a toast when a new release is out.</span>
          </div>
        </div>
      </div>
      ...
      <div class="alert-box tech mt-3">
        ...
        <ul class="custom-list">
          ...
          <li><strong>App-icon menu & window presets:</strong> The ☰ titlebar menu holds View Changelog (moved out of the clickable titlebar version, which is now plain text), update checks, SSH Config, Enable SSH Terminal Color, a <strong>Glass Effect</strong> toggle (frosted-glass blur on translucent chrome, off by default - costs GPU when on), Statusline Customizer, Claude Code Cleanup, Remote Control, a <code>Usage slots:</code> picker (2/4/6/8), and window presets (<code>F1</code> Narrow+Stick Top-Left on the current monitor, <code>F2</code> toggle Ultra 1400px/Narrow, <code>F3</code> Center Primary, <code>F12</code> toggle Pin). These four F-keys still reach the app while the in-app terminal is focused at a shell prompt - a full-screen program (<code>vim</code>, <code>htop</code>) keeps them instead. Tick <strong>remember</strong> to restore the exact window size/position next launch.</li>
