    ...
    <div class="modal-body scrollable intro-content u-select-text">
      ...
      <div class="alert-box tech mt-3">
        ...
        <ul class="custom-list">
          ...
          <li><strong>App-icon menu & window presets:</strong> The ☰ titlebar menu collects GitHub/Release links, manual update check, SSH Config, Enable SSH Terminal Color, Statusline Customizer, Claude Code Profile (Local), Claude Code Cleanup (Local) - what <code>~/.claude</code> costs in disk, with a checkbox per path so you tick exactly what goes; skills, hooks and settings are never deletable, and agent memory sits in its own group so clearing chat history can't take it along - Pre-allow AGY Commands (seeds a recommended command allowlist into agy's <code>settings.json</code> on local/remote hosts, so it stops prompting for permission on routine commands), Remote Control, the AkiClaudeDoc installer, a <code>Usage row:</code> picker for 1 row (2 slots) or 2 rows (4 slots), and 4 window presets under <code>AppWindow:</code> - Narrow (420px), Wide (768px), Stick Top-Left (auto-fits height to the project list), Center Primary. <code>⌘1</code> = Narrow + Stick Top-Left, <code>⌘2</code> = Wide + Center Primary (both work without opening the menu). Tick <strong>remember</strong> to re-apply the width and placement you chose on the next launch (width first, since Stick Top-Left fits its height to the current width).</li>
