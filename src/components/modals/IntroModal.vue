    ...
    <div class="modal-body scrollable intro-content u-select-text">
      ...
      <div class="alert-box tech mt-3">
        ...
        <ul class="custom-list">
          ...
          <li><strong>App-icon menu & window presets:</strong> The ☰ titlebar menu collects GitHub/Release links, manual update check, SSH Config, Enable SSH Terminal Color, Statusline Customizer, Claude Code Profile (Local), Claude Code Cleanup (Local) - what <code>~/.claude</code> costs in disk, with a checkbox per path so you tick exactly what goes; skills, hooks and settings are never deletable, and agent memory sits in its own group so clearing chat history can't take it along - Remote Control, the AkiClaudeDoc installer, a <code>Usage row:</code> picker for 1 row (2 slots) or 2 rows (4 slots), and 4 window presets under <code>AppWindow:</code> - Narrow (420px), Wide (768px), Stick Top-Left (auto-fits height to the project list), Center Primary. <code>⌘1</code> = Narrow + Stick Top-Left, <code>⌘2</code> = Wide + Center Primary (both work without opening the menu). Tick <strong>remember</strong> to re-apply the width and placement you chose on the next launch (width first, since Stick Top-Left fits its height to the current width).</li>
          ...
          <li><strong>Vietnamese typing in the terminal:</strong> two different problems, two fixes. OpenKey/EVKey are not real IMEs - they retype each syllable as a burst of raw synthetic keystrokes, which Tauri's WKWebView tags as "IME keys" (keyCode 229) and xterm's fallback code then drops or half-deletes (an upstream xterm.js bug Chromium never triggers - why VS Code was always fine). A capture-phase guard now hands those keystrokes straight to the shell, so <em>direct typing with OpenKey works</em>. macOS's own Vietnamese input is a true composing IME whose preedit cannot live against a no-local-echo terminal (the PTY is the single source of truth; a character is only drawn once the shell echoes it back) - for it, compose in the real <code>&lt;input&gt;</code> under the key row, which owns the text with synchronous feedback and sends only the finished line - that row renders on the Mac too.</li>
