<template>
  <div v-if="showSpecialModal" class="modal-overlay">
    <div class="modal-container special-modal">
      <div class="modal-header header-special">
        <h2><i class="fa-solid fa-list-check mr-1"></i> Push Special: {{ specialProject?.name }}</h2>
        <button class="btn-close-modal" @click="closeSpecialModal"><i class="fa-solid fa-xmark"></i></button>
      </div>
...
function handleEsc(e) {
  if (e.key === 'Escape' && showSpecialModal.value) {
    closeSpecialModal();
  }
}
