...
export function useAppWindow() {
  ...
  /** Sets width to widthLogical, keeps height; nudges x back on-screen only if the new width would push past the monitor edge. */
  async function setWidthPreset(widthLogical) {
    ...
    if (x + widthLogical > maxX) x = maxX - widthLogical;
    if (x < minX) x = minX;
  ...
  /** ⌘1/⌘2 fire a full grid column (both axes) so one keystroke lands a complete layout. */
  const VIEW_COMBOS = {
