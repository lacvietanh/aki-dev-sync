...
| WS-C | Usage probe OOP: `get-antigravity-usage.js` → shell, delete `Interpreter::Node`, split `agent_usage.rs` (892 lines) into domain modules behind a shared `UsageProbe` trait | `done/usage-probe-oop.md` | **implemented** (the trait was argued and rejected — see that doc §5) |
