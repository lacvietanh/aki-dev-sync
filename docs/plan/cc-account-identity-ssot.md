...
Status: **code fixed, pending rebuild+Apply+owner verify** (Fix 1, 2, 4 in `scripts/get-claudecode-usage.sh` / `src-tauri/src/statusline-unified.sh`, Fix 3 in `scripts/provision-claudecode.sh` — all on the working tree, uncommitted) · Verified by reproducing the account-switch scenario against a sandboxed `$HOME` and diffing the pre-fix script's output against the working-tree script's output: pre-fix emits the stale `AUTHINFO` email while the quota cache already carries the new account's numbers (exactly the reported symptom); the working-tree script emits the live email in the same run. `src-tauri/src/agent_usage/claudecode.rs` and the JS usage store needed **no change** — both already treat every poll's `AUTHINFO`/`content` as the full, authoritative replacement rather than a cache to merge selectively. **See §14 — a second, more fundamental path bug was found on 2026-07-30 after this plan's fix landed, and is fixed on top of it; the app has not yet been rebuilt+Applied+owner-verified against the corrected path.**
...
**Fixed on top of §5's fix (uncommitted at the time of writing):**
...
**Still open when this addendum was written:** the fix requires a `npm run tauri dev` restart (the script is `include_str!`-embedded in the Rust binary — editing the file on disk does not propagate to a running build) **and** a fresh Statusline Customizer → Apply for Claude Code (the template embedded in the binary is not auto-installed; `~/.claude/statusline-command.sh` is a separate installed copy). Until both happen, `grep -n "_live_claude_json" ~/.claude/statusline-command.sh` returns nothing and the owner will keep seeing the old behavior. Full acceptance test: switch CC account, run one command in a CC terminal (statusLine hook fires), click Refresh in the app, and confirm both the app card and the real terminal statusline show the new email.
