# Verify-pending — CLOSED, all items confirmed passed on Mac 2026-08-15

All 11 items below were code-complete and committed, blocked only on a human sitting at the Mac (or a phone against it) and observing the result. The owner manually confirmed on 2026-08-15 that every item — T1, T2, T3, T4, T5, T6, U1, U2, U4, I1, I2 — passed. This file is now closed as a historical record.
...
**Result:** Resolved — confirmed by owner on Mac, 2026-08-15. All steps passed.
...
### T6 — Manual resize-authority handoff (Step 8)
**Changed:** `FRAME_PTY_RESIZE_REQUEST` (`src/constants/protocol.js:135`), `resizeOwner`/`reclaimResizeAuthority` (`src/store/terminalTabsStore.js:207-219`), host listener + `ownsPtySize` (`src/composables/usePtyTerminal.js:433,589-595,665`), "Fit to my screen" button + reclaim pill (`src/components/TerminalView.vue:65,75,307`). Landed and shipped 2026-08-12/`1.24.0`.
**Source:** `docs/plan/done/wish-terminal-manual-resize-authority.md`, "Smallest correct shippable V1" Step 8. Had no tracking row here until 2026-08-15 (`docs/plan/remaining-1.25.md` item 5).
**Steps:** On a companion (phone), tap "Fit to my screen" in a plain shell — the Mac must stop auto-driving and the phone's geometry must apply; repeat inside a full TUI (`claude`/`agy`/`vim`). On the Mac, tap the reclaim pill — the Mac must resume auto-driving and the companion's tap must no longer override it. Confirm no hijack: a companion cannot silently take authority without the explicit tap.
**Result:** Resolved — confirmed by owner on Mac, 2026-08-15. All steps passed.
...
**Changed:** `src/composables/useDockLayout.js`, `src/components/DockStack.vue`, `src/components/AppConsole.vue`, `src/assets/main.css` — the dock's stored total height replaced by a per-stack length map summed into `dockHeightCss`; each expanded stack renders its own splitter. Landed 2026-08-12, committed (`33a94c6`) and shipped in `1.24.0`.
**Source:** `docs/plan/done/dock-stack-independent-height.md` §4 (full check list, seven items).
