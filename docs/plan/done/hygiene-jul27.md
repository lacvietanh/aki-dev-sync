...
Status (2026-07-31, plan-consolidation pass): **2 of 3 closed, item 3 partially closed (docs bucket done, JS/Vue + Rust buckets still open) — this doc stays active only for item 3's remaining code-bucket work.**

- **Item 1 (WebSocket reconnect noise) — closed, no code change.** Runtime confirmation tracked centrally: [`verify-pending.md` §Infra I1](verify-pending.md#i1--companion-websocket-reconnect-convergence-on-real-sleepwake).
- **Item 2 (project-icon 404) — fix applied and committed** (`06a3ad3`). Runtime confirmation tracked centrally: [`verify-pending.md` §Infra I2](verify-pending.md#i2--project-icon-404-avoidance).
- **Item 3 (hard-wrap sweep) — settled 2026-07-31, PARTIALLY finished: docs bucket landed, code buckets did not.** Evidence: `git show --stat 1c00e3b 3c5188e` — `1c00e3b` (2026-07-28) touched 47 `docs/**/*.md` files (one entry is `1.21.1-terminal-limits-and-structure.md` moving to `done/`, counted once) plus `docs/index.md`; `3c5188e` (same day) touched `CLAUDE.md` alone. 47 + 1 (`CLAUDE.md`) = 48, and `docs/index.md`'s own edit brings the touched-file count for the 07-30 status line's "49 doc files" within one of exact — strong corroboration that the 07-30 line was a stale/mis-dated redescription of this already-committed work, not a phantom second pass on top of it. Sample check (this session): `docs/plan/done/dev-build-in-app-launch.md`, `docs/plan/done/dev-build-visibility.md`, `docs/feat/agy-command-allowlist.md` still show the ≥3-line ~90-101-col wrap signature — but `git log` shows all three postdate the sweep (created 2026-07-30, two days after `1c00e3b`/`3c5188e`), so they were never in-scope for it; not evidence of a missed pass. `docs/plan/done/total-refactor-by-akirule.md` (last touched 2026-07-22, predates the sweep) and `docs/plan/done/remote-control.md` (touched BY `1c00e3b` itself) each still carry 1-2 genuinely wrapped paragraph runs — a handful of real misses inside the swept docs bucket, not proof the bucket is untouched. `docs/research/test-case-check-flow.md` also still shows the pattern (23 runs) — out of scope for this agent to touch or judge further (owned by a concurrent agent). **What is NOT done, confirmed by direct read**: the sweep commits touched `.md`/`CLAUDE.md` files only — zero `src/**/*.js`/`*.vue` or `src-tauri/src/*.rs` files appear in either commit's stat. Spot-checked `src/services/bridge.js:13-17` and `src-tauri/src/web_server.rs` (64 lines cluster at length 98-99) — both still carry the exact hard-wrap signature §3 originally flagged (67 JS/Vue files + 13 Rust files, ~2,239 lines combined, per §3's table below). **Verdict**: the docs-bucket portion of item 3 is finished — closing it here — but the JS/Vue and Rust buckets were never attempted and remain fully open work, not a rounding error. §3's inventory below is the pre-existing triage and is left untouched.

This doc stays active: item 3's code-bucket work (JS/Vue + Rust hard-wrap, ~80 files) is still open. Everything below is the original triage.

Owner workstream for `docs/plan/done/backlog-jul27.md` WS-E. Three unrelated items triaged together because none blocks or is blocked by the other workstreams. This doc is a **triage report** (agent.B5): findings and verdicts only — nothing in the tree has been touched to produce it.
...
**Still unverified**: runtime confirmation moved to [`verify-pending.md` §Infra I1](verify-pending.md#i1--companion-websocket-reconnect-convergence-on-real-sleepwake).
...
- **(b) Comment vs `agent.C3` and `docs.B3`** — measuring this file's own pre-existing comments (the header block and the `assetBase` comment, both already wrapped ~94-101 cols before this edit), the new block's wrap column was not out of line with the file's local convention, so no C3 rewrap was needed on that axis. The real problem was length and duplication: the added ~11-line block re-narrated this section's own diagnosis (WebKit's 404 log, the Rust fallback, the "already known" reasoning) inline in source, which is exactly what `docs.B3` says not to do — a doc (this one) already carries that reasoning in detail, so the comment should point at it, not restate it. Rewrote the block down to ~5 lines that state the *what* and link to `docs/plan/done/hygiene-jul27.md §2` for the *why*.
...
**Remains unverified**: runtime confirmation moved to [`verify-pending.md` §Infra I2](verify-pending.md#i2--project-icon-404-avoidance).
...
- `docs/plan/done/backlog-jul27.md` contains a markdown table (workstream index) — table rows are structurally atomic; never rejoin or rewrap them.
