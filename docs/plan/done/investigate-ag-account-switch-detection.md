...
**Not the same defect as `docs/plan/done/ag-usage-pin-vs-live.md`, despite both being "AG shows the wrong account/quota".** This plan is about a live `language_server` process that keeps returning the *old* account's context after an in-IDE switch, because Antigravity may not restart it and its CSRF token is issued at process start. That one is about a *fresh, correct* reading being discarded downstream by a pin that demands a session type which is not live. Closing either one does not close the other.
