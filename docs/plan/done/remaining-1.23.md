...
- **PLAN-SPLIT** — decided: keep `docs/plan/wish-terminal-split-simpleview.md` as one file with a two-milestone status header (SimpleView ✅ built/1.23.0, Right-Dock still unbuilt), **not** moved to `done/`. Splitting a doc whose Right-Dock half is live wishlist adds extraction risk for little gain.
