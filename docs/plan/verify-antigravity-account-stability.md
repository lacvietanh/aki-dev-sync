...
Status: **TODO** (user sẽ tự chạy debug + test, báo cáo kèm log)
...
- **B. `viewingEmail` auto-reset** (item 2) — lỗ hổng cùng loại, chưa vá.
...
## A. Test protocol — xác nhận fix `a26b8f5` (item 1)
...
## C. Phân loại lý do "offline" ở tầng Rust/script (item 3 — chưa implement, đề xuất)
