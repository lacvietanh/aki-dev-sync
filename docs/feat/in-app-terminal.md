...
**Right-Dock Layout (1.25).** When the window width is ≥ 900px, the terminal stack moves out of the bottom dock into a dedicated right-side column taking 100% of remaining width (`flex: 1`), with the main project list capped at 420px. In this mode, the terminal is dedicated (collapse/maximize buttons are hidden) and the LogStack docks at the bottom of the main view column. When the window is < 900px, the layout returns to the bottom dock with independent collapse/maximize controls.
