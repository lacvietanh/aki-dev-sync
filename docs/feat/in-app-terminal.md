...
`useTerminalViewType.js` now returns `TerminalView.vue` on every screen; resize safety comes from the explicit, revocable authority handoff instead (`docs/plan/wish-terminal-manual-resize-authority.md`). `SimpleView.vue` stays in the tree, unreferenced, as the candidate for a future opt-in low-bandwidth mode — which is the only reason the seam still exists rather than `TerminalStack.vue` importing `TerminalView` directly.
