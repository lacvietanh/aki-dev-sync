...
**Excludes.** Since 1.24.0, `.akidevsync/` is no longer excluded from either direction — the owner wants notes/metadata to travel with the project, and a one-time `migrateStripNotesExcludes` removes the entry from existing projects' lists. Residual risk, accepted: a project that was PULLed before it was ever PUSHed has no `.akidevsync/` on the remote yet, so a mirror PULL (`delete_on_pull` defaults to `true`, which passes `--delete`) removes the local one. That risk is now closed: whenever `build_rsync_args` (`src-tauri/src/sync.rs`) adds `--delete` for a mirror direction, it also adds `--filter=P .akidevsync/`, an rsync protect rule that stops `--delete` from ever removing the receiver's `.akidevsync/` while leaving normal transfer of it untouched.
...
- `src/composables/useProjectConfig.js` - new-project defaults, `migrateStripNotesExcludes`, and the hydrate + migrate sequence inside `loadData`.
