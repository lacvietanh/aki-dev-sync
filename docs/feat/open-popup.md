...
- Placement is **CSS Anchor Positioning** (`main.css` `.open-popup`), not JS: each trigger sets a unique `anchor-name` inline (per-row, since there is one per project id), and the popup targets it via `position-anchor`. No `getBoundingClientRect`/measurement JS is involved — this replaced an earlier `anchorPopover()` JS function that measured and clamped coordinates by hand.
  - Default anchors the popup's **right** edge to the trigger's right edge, bottom to the trigger's top (a drop-up, correct for a trigger that has room to its left). ProjectTable's OPEN button is the **first/leftmost** action in its cell, so it gets the `.anchor-left` modifier instead — the popup's **left** edge anchors to the trigger's left edge, growing rightward into the row rather than off the left edge of a narrow right-dock column. `position-try-fallbacks`/`@position-try` let the engine flip sides on its own if the chosen edge would still overflow (browser support permitting).
