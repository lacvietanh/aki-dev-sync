...
- Placement is **CSS Anchor Positioning** (`main.css` `.open-popup`), not JS: each trigger sets a unique `anchor-name` inline (per-row, since there is one per project id), and the popup targets it via `position-anchor`. No `getBoundingClientRect`/measurement JS is involved — this replaced an earlier `anchorPopover()` JS function that measured and clamped coordinates by hand.
  - **Vertical** placement uses `anchor()`: default is a drop-up (`bottom: calc(anchor(top) + 4px)`), since that only needs to know the trigger's row.
  - **Horizontal** placement is per-caller, because the two call sites' triggers sit in different places relative to the available width. Default anchors the popup's **right** edge to the trigger's right edge (`TerminalChromeMenu.vue`'s trigger sits at the right end of the header actions row, room to grow leftward). ProjectTable's OPEN button sits **mid-row**, not at either screen edge — anchoring the popup to it (either edge) pushed half the popup off-screen in the narrow right-dock column, since half of it always lands past whichever side the row happens to be shorter on. Its popup gets the `.pin-left` modifier instead: a plain `left: 8px`, viewport-relative and independent of the trigger entirely (`position: fixed` is already viewport-relative, so this axis needs no `anchor()` at all) — it always opens flush against the app's own left edge.
- The same shell serves `TerminalChromeMenu.vue`, which drops **down** instead of up (`.is-dropdown`) whenever there isn't reliably room above the trigger: right-dock mode, or the terminal panel maximized (trigger sits near the top of the window then).
