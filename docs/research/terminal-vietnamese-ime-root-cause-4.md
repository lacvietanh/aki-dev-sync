...
**Cross-references** — `terminal-vietnamese-ime-root-cause-3.md` (given its `Status: superseded by` line by this round), `docs/plan/done/terminal-ime-input-layer-separation.md` (its §1 "what VS Code actually does" is corrected by §1-§3 here; its §3 built event-payload reading where its own §1 named value reading), `docs/plan/terminal-input-surface.md` (§5.1's VS Code framing and the compose-row work in §2/§3), `docs/feat/in-app-terminal.md` §Vietnamese input, `docs/index.md` (chain head moves to this doc), `src/composables/useTerminalInput.js` and `src/composables/useWkImeGuard.js` (both superseded by the §7 design), `docs/plan/backlog-jul27.md` WS-D (no longer blocked on Mac evidence).
